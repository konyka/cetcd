...
CETCD_TEST_CASE(tls_set_ciphers_tls13_iana) {
    cetcd_tls_ctx *ctx = cetcd_tls_ctx_new();
    CETCD_ASSERT_NOT_NULL(ctx);
    CETCD_ASSERT_EQ_INT(cetcd_tls_set_ciphers(ctx, "TLS_AES_256_GCM_SHA384"),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_tls_set_ciphers(ctx,
        "TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384,TLS_AES_128_GCM_SHA256"),
                        CETCD_OK);
    CETCD_ASSERT_NE_INT(cetcd_tls_set_ciphers(ctx, "TLS_AES_FOO_SHA256"),
                        CETCD_OK);
    cetcd_tls_ctx_free(ctx);
}

CETCD_TEST_CASE(tls_set_ciphers_tls13_handshake) {
    char dir[128];
    CETCD_ASSERT_EQ_INT(make_selfsigned_(dir, sizeof(dir)), 0);
    char cert[300], key[300];
    snprintf(cert, sizeof(cert), "%s/cert.pem", dir);
