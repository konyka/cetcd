...
CETCD_TEST_CASE(apply_auth_role_delete_then_gone) {
    cetcd_v3rpc *rpc = cetcd_v3rpc_new();
    uint8_t *buf = NULL;
    size_t len = 0;
    CETCD_ASSERT_TRUE(cetcd_apply_encode_auth_role_delete(&buf, &len, NULL, 0) != 0);
    uint8_t trunc[] = { CETCD_APPLY_AUTH_ROLE_DELETE };
    CETCD_ASSERT_TRUE(cetcd_v3rpc_apply_entry(trunc, sizeof(trunc)) != 0);

    CETCD_ASSERT_EQ_INT(cetcd_apply_encode_auth_role_delete(&buf, &len,
        (const uint8_t *)"admin", 5), 0);
    CETCD_ASSERT_EQ_INT((int)buf[0], CETCD_APPLY_AUTH_ROLE_DELETE);
    CETCD_ASSERT_EQ_INT(cetcd_v3rpc_apply_entry(buf, len), 0);
    free(buf);
...
CETCD_TEST_CASE(apply_auth_user_grant_role_then_get) {
    cetcd_v3rpc *rpc = cetcd_v3rpc_new();
    uint8_t *buf = NULL;
    size_t len = 0;
    CETCD_ASSERT_TRUE(cetcd_apply_encode_auth_user_grant_role(&buf, &len,
        NULL, 0, (const uint8_t *)"admin", 5) != 0);
    uint8_t trunc[] = { CETCD_APPLY_AUTH_USER_GRANT_ROLE };
    CETCD_ASSERT_TRUE(cetcd_v3rpc_apply_entry(trunc, sizeof(trunc)) != 0);

    CETCD_ASSERT_EQ_INT(cetcd_apply_encode_auth_user_grant_role(&buf, &len,
        (const uint8_t *)"root", 4, (const uint8_t *)"admin", 5), 0);
    CETCD_ASSERT_EQ_INT((int)buf[0], CETCD_APPLY_AUTH_USER_GRANT_ROLE);
    ...
    cetcd_rpc_bytes resp = cetcd_v3rpc_dispatch(rpc,
        "/etcdserverpb.Auth/UserGet", get_buf, pos);
    CETCD_ASSERT_NOT_NULL(resp.data);
    int found = 0;
...
CETCD_TEST_CASE(apply_auth_user_revoke_role_then_gone) {
    cetcd_v3rpc *rpc = cetcd_v3rpc_new();
    uint8_t *buf = NULL;
    size_t len = 0;
    CETCD_ASSERT_TRUE(cetcd_apply_encode_auth_user_revoke_role(&buf, &len,
        NULL, 0, (const uint8_t *)"admin", 5) != 0);
    uint8_t trunc[] = { CETCD_APPLY_AUTH_USER_REVOKE_ROLE };
    CETCD_ASSERT_TRUE(cetcd_v3rpc_apply_entry(trunc, sizeof(trunc)) != 0);

    CETCD_ASSERT_EQ_INT(cetcd_apply_encode_auth_user_revoke_role(&buf, &len,
        (const uint8_t *)"root", 4, (const uint8_t *)"admin", 5), 0);
    CETCD_ASSERT_EQ_INT((int)buf[0], CETCD_APPLY_AUTH_USER_REVOKE_ROLE);
    CETCD_ASSERT_EQ_INT(cetcd_v3rpc_apply_entry(buf, len), 0);
    free(buf);
...
CETCD_TEST_CASE(apply_auth_role_revoke_perm_then_gone) {
    cetcd_v3rpc *rpc = cetcd_v3rpc_new();
    uint8_t *buf = NULL;
    size_t len = 0;
