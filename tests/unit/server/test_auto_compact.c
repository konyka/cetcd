...
CETCD_TEST_CASE(auto_compact_parse_status_is_learner) {
    uint8_t buf[16];
    size_t n = 0;
    int learner = 9;

    CETCD_ASSERT_EQ_INT(cetcd_encode_status_is_learner(0, buf, sizeof(buf), &n),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(n == 0);
    CETCD_ASSERT_EQ_INT(cetcd_encode_status_is_learner(1, buf, sizeof(buf), &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_status_is_learner(buf, n, &learner),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(learner == 1);

    CETCD_ASSERT_EQ_INT(cetcd_parse_status_is_learner(NULL, 0, &learner),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(learner == 0);
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_status_is_learner(dummy, 1, &learner),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(learner == 0);

    /* leftover truncated isLearner cannot look like a voter */
    uint8_t trunc[] = { 0x50 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_status_is_learner(trunc, 1, &learner),
                        CETCD_ERR_INVAL);

    /* leftover cannot steal a printed isLearner */
    uint8_t steal[] = { 0x1a, 0x02, 0x50, 0x01 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_status_is_learner(steal, sizeof(steal),
                                                     &learner),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(learner == 0);

    uint8_t last[] = { 0x50, 0x01, 0x50, 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_status_is_learner(last, sizeof(last),
                                                     &learner),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(learner == 0);

    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_status_is_learner(fixed64, 1, &learner),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_status_is_learner(buf, n, NULL),
                        CETCD_ERR_INVAL);
}
...
CETCD_TEST_CASE(auto_compact_parse_status_raft_term) {
    uint8_t buf[16];
    size_t n = 0;
    uint64_t term = 9;

    CETCD_ASSERT_EQ_INT(cetcd_encode_status_raft_term(0, buf, sizeof(buf), &n),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(n == 0);
    CETCD_ASSERT_EQ_INT(cetcd_encode_status_raft_term(3, buf, sizeof(buf), &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_status_raft_term(buf, n, &term), CETCD_OK);
    CETCD_ASSERT_TRUE(term == 3);

    CETCD_ASSERT_EQ_INT(cetcd_parse_status_raft_term(NULL, 0, &term), CETCD_OK);
    CETCD_ASSERT_TRUE(term == 0);
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_status_raft_term(dummy, 1, &term),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(term == 0);

    /* leftover truncated raftTerm cannot look like raftTerm 0 */
    uint8_t trunc[] = { 0x30 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_status_raft_term(trunc, 1, &term),
                        CETCD_ERR_INVAL);

    /* leftover cannot steal a printed raftTerm */
    uint8_t steal[] = { 0x1a, 0x02, 0x30, 0x03 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_status_raft_term(steal, sizeof(steal),
                                                    &term),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(term == 0);

    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_status_raft_term(fixed64, 1, &term),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_status_raft_term(buf, n, NULL),
                        CETCD_ERR_INVAL);
}

CETCD_TEST_CASE(auto_compact_parse_status_raft_applied) {
    uint8_t buf[16];
    size_t n = 0;
    uint64_t applied = 9;

    CETCD_ASSERT_EQ_INT(cetcd_encode_status_raft_applied(0, buf, sizeof(buf),
                                                        &n),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(n == 0);
    CETCD_ASSERT_EQ_INT(cetcd_encode_status_raft_applied(11, buf, sizeof(buf),
                                                        &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_status_raft_applied(buf, n, &applied),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(applied == 11);

    CETCD_ASSERT_EQ_INT(cetcd_parse_status_raft_applied(NULL, 0, &applied),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(applied == 0);
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_status_raft_applied(dummy, 1, &applied),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(applied == 0);

    /* leftover truncated raftAppliedIndex cannot look like 0 */
    uint8_t trunc[] = { 0x38 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_status_raft_applied(trunc, 1, &applied),
                        CETCD_ERR_INVAL);

    /* leftover cannot steal a printed raftAppliedIndex */
    uint8_t steal[] = { 0x1a, 0x02, 0x38, 0x0b };
    CETCD_ASSERT_EQ_INT(cetcd_parse_status_raft_applied(steal, sizeof(steal),
                                                       &applied),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(applied == 0);

    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_status_raft_applied(fixed64, 1, &applied),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_status_raft_applied(buf, n, NULL),
                        CETCD_ERR_INVAL);
}
...
CETCD_TEST_CASE(auto_compact_parse_role_get_response) {
    uint8_t buf[32];
    size_t n = 0, got = 9;
    int perm = 9;
    char key[16];

    CETCD_ASSERT_EQ_INT(cetcd_encode_role_get_perm(1, NULL, buf, sizeof(buf),
                                                   &n),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_encode_role_get_perm(1, "/k", buf, sizeof(buf),
                                                   &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_role_get_response(buf, n, &perm, key,
                                                      sizeof(key), &got),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(perm, 1);
    CETCD_ASSERT_EQ_STR(key, "/k");
    CETCD_ASSERT_TRUE(got == 1);

    CETCD_ASSERT_EQ_INT(cetcd_parse_role_get_response(NULL, 0, &perm, key,
                                                      sizeof(key), &got),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(key, "");
    CETCD_ASSERT_TRUE(got == 0);
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_role_get_response(dummy, 1, &perm, key,
                                                      sizeof(key), &got),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(got == 0);

    /* leftover truncated perm cannot print a leftover key */
    uint8_t trunc[] = { 0x12 };
    ...
    /* leftover cannot steal a printed key */
    uint8_t steal[] = { 0x1a, 0x06, 0x12, 0x04, 0x0a, 0x02, '/', 'k' };
    CETCD_ASSERT_EQ_INT(cetcd_parse_role_get_response(steal, sizeof(steal),
                                                      &perm, key, sizeof(key),
                                                      &got),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(got == 0);
    ...
    uint8_t steal_pt[] = { 0x12, 0x04, 0x1a, 0x02, 0x08, 0x01 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_role_get_response(steal_pt,
                                                      sizeof(steal_pt),
                                                      &perm, key, sizeof(key),
                                                      &got),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(perm, 0);

    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_role_get_response(fixed64, 1, &perm, key,
                                                      sizeof(key), &got),
                        CETCD_ERR_INVAL);
...
CETCD_TEST_CASE(auto_compact_parse_role_get_perm_type) {
    uint8_t buf[16];
    size_t n = 0;
    int perm_type = 9;

    CETCD_ASSERT_EQ_INT(cetcd_encode_role_get_perm_type(-1, buf, sizeof(buf),
                                                       &n),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_encode_role_get_perm_type(0, buf, sizeof(buf),
                                                       &n),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(n == 0);
    CETCD_ASSERT_EQ_INT(cetcd_encode_role_get_perm_type(1, buf, sizeof(buf),
                                                       &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_role_get_perm_type(buf, n, &perm_type),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(perm_type, 1);

    CETCD_ASSERT_EQ_INT(cetcd_parse_role_get_perm_type(NULL, 0, &perm_type),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(perm_type, 0);
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_role_get_perm_type(dummy, 1, &perm_type),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(perm_type, 0);

    /* leftover truncated permType cannot look like READ */
    uint8_t trunc[] = { 0x12, 0x01, 0x08 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_role_get_perm_type(trunc, sizeof(trunc),
                                                      &perm_type),
                        CETCD_ERR_INVAL);

    /* leftover cannot steal a printed permType */
    uint8_t steal[] = { 0x12, 0x04, 0x32, 0x02, 0x08, 0x01 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_role_get_perm_type(steal, sizeof(steal),
                                                      &perm_type),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(perm_type, 0);

    /* leftover header cluster_id is not permType */
    uint8_t header[] = { 0x0a, 0x02, 0x08, 0x01 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_role_get_perm_type(header, sizeof(header),
                                                      &perm_type),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(perm_type, 0);

    uint8_t last[] = { 0x12, 0x02, 0x08, 0x01, 0x12, 0x02, 0x08, 0x02 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_role_get_perm_type(last, sizeof(last),
                                                      &perm_type),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(perm_type, 2);

    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_role_get_perm_type(fixed64, 1, &perm_type),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_role_get_perm_type(buf, n, NULL),
                        CETCD_ERR_INVAL);
}
...
CETCD_TEST_CASE(auto_compact_parse_watch_response) {
    ...
    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_kv(0, NULL, buf, sizeof(buf),
                                                    &n),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_kv(0, "/w", buf, sizeof(buf),
                                                    &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_response(buf, n, &type, key,
                                                   sizeof(key), &got),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(type, 0);
    CETCD_ASSERT_EQ_STR(key, "/w");
    CETCD_ASSERT_TRUE(got == 1);

    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_response(NULL, 0, &type, key,
                                                   sizeof(key), &got),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(key, "");
    CETCD_ASSERT_TRUE(got == 0);
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_response(dummy, 1, &type, key,
                                                   sizeof(key), &got),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(got == 0);

    /* leftover truncated event cannot print a leftover key */
    uint8_t trunc[] = { 0x5a };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_response(trunc, 1, &type, key,
                                                   sizeof(key), &got),
                        CETCD_ERR_INVAL);
    uint8_t trunc_kv[] = { 0x5a, 0x01, 0x12 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_response(trunc_kv, sizeof(trunc_kv),
                                                   &type, key, sizeof(key),
                                                   &got),
                        CETCD_ERR_INVAL);

    /* leftover cannot steal a printed watch key */
    uint8_t steal[] = { 0x1a, 0x08, 0x5a, 0x06, 0x12, 0x04, 0x0a, 0x02, '/',
                        'w' };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_response(steal, sizeof(steal), &type,
                                                   key, sizeof(key), &got),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(got == 0);
    CETCD_ASSERT_EQ_STR(key, "");

    uint8_t inner_steal[] = { 0x5a, 0x08, 0x12, 0x06, 0x1a, 0x04, 0x0a, 0x02,
                              '/', 'w' };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_response(inner_steal,
                                                   sizeof(inner_steal),
                                                   &type, key, sizeof(key),
                                                   &got),
                        CETCD_OK);
    ...
    uint8_t steal_type[] = { 0x5a, 0x04, 0x22, 0x02, 0x08, 0x01 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_response(steal_type,
                                                   sizeof(steal_type),
                                                   &type, key, sizeof(key),
                                                   &got),
                        CETCD_OK);
    ...
    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_response(fixed64, 1, &type, key,
                                                   sizeof(key), &got),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_response(buf, n, NULL, NULL, 0, NULL),
                        CETCD_ERR_INVAL);
}

CETCD_TEST_CASE(auto_compact_parse_watch_event_kv_lease) {
    uint8_t buf[16];
    size_t n = 0;
    int64_t lease = 9;

    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_kv_lease(0, buf, sizeof(buf),
                                                         &n),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(n == 0);
    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_kv_lease(5, buf, sizeof(buf),
                                                         &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_lease(buf, n, &lease),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(lease == 5);

    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_lease(NULL, 0, &lease),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(lease == 0);
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_lease(dummy, 1, &lease),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(lease == 0);

    /* leftover truncated Event-nested lease cannot look like 0 */
    uint8_t trunc[] = { 0x5a, 0x03, 0x12, 0x01, 0x30 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_lease(trunc, sizeof(trunc),
                                                        &lease),
                        CETCD_ERR_INVAL);

    /* leftover cannot steal a printed Event lease */
    uint8_t steal[] = { 0x5a, 0x06, 0x12, 0x04, 0x32, 0x02, 0x30, 0x05 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_lease(steal, sizeof(steal),
                                                        &lease),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(lease == 0);

    /* leftover header raft_term is not Event lease */
    uint8_t header[] = { 0x0a, 0x02, 0x30, 0x07 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_lease(header, sizeof(header),
                                                        &lease),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(lease == 0);

    uint8_t last[] = { 0x5a, 0x04, 0x12, 0x02, 0x30, 0x01,
                       0x5a, 0x04, 0x12, 0x02, 0x30, 0x02 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_lease(last, sizeof(last),
                                                        &lease),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(lease == 2);

    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_lease(fixed64, 1, &lease),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_lease(buf, n, NULL),
                        CETCD_ERR_INVAL);
}

CETCD_TEST_CASE(auto_compact_parse_watch_event_kv_version) {
    uint8_t buf[16];
    size_t n = 0;
    int64_t version = 9;

    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_kv_version(0, buf, sizeof(buf),
                                                           &n),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(n == 0);
    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_kv_version(3, buf, sizeof(buf),
                                                           &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_version(buf, n, &version),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(version == 3);

    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_version(NULL, 0, &version),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(version == 0);
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_version(dummy, 1, &version),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(version == 0);

    /* leftover truncated Event-nested version cannot look like 0 */
    uint8_t trunc[] = { 0x5a, 0x03, 0x12, 0x01, 0x20 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_version(trunc, sizeof(trunc),
                                                          &version),
                        CETCD_ERR_INVAL);

    /* leftover cannot steal a printed Event version */
    uint8_t steal[] = { 0x5a, 0x06, 0x12, 0x04, 0x32, 0x02, 0x20, 0x03 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_version(steal, sizeof(steal),
                                                          &version),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(version == 0);

    /* leftover header raft_term is not Event version */
    uint8_t header[] = { 0x0a, 0x02, 0x20, 0x07 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_version(header,
                                                          sizeof(header),
                                                          &version),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(version == 0);

    uint8_t last[] = { 0x5a, 0x04, 0x12, 0x02, 0x20, 0x01,
                       0x5a, 0x04, 0x12, 0x02, 0x20, 0x02 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_version(last, sizeof(last),
                                                          &version),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(version == 2);

    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_version(fixed64, 1, &version),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_version(buf, n, NULL),
                        CETCD_ERR_INVAL);
}

CETCD_TEST_CASE(auto_compact_parse_watch_event_kv_create_rev) {
    uint8_t buf[16];
    size_t n = 0;
    int64_t create_rev = 9;

    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_kv_create_rev(0, buf,
                                                              sizeof(buf),
                                                              &n),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(n == 0);
    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_kv_create_rev(4, buf,
                                                              sizeof(buf),
                                                              &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_create_rev(buf, n,
                                                             &create_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(create_rev == 4);

    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_create_rev(NULL, 0,
                                                             &create_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(create_rev == 0);
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_create_rev(dummy, 1,
                                                             &create_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(create_rev == 0);

    /* leftover truncated Event-nested create_revision cannot look like 0 */
    uint8_t trunc[] = { 0x5a, 0x03, 0x12, 0x01, 0x10 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_create_rev(trunc,
                                                             sizeof(trunc),
                                                             &create_rev),
                        CETCD_ERR_INVAL);

    /* leftover cannot steal a printed Event create_revision */
    uint8_t steal[] = { 0x5a, 0x06, 0x12, 0x04, 0x32, 0x02, 0x10, 0x04 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_create_rev(steal,
                                                             sizeof(steal),
                                                             &create_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(create_rev == 0);

    /* leftover header member_id is not Event create_revision */
    uint8_t header[] = { 0x0a, 0x02, 0x10, 0x03 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_create_rev(header,
                                                             sizeof(header),
                                                             &create_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(create_rev == 0);

    uint8_t last[] = { 0x5a, 0x04, 0x12, 0x02, 0x10, 0x01,
                       0x5a, 0x04, 0x12, 0x02, 0x10, 0x02 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_create_rev(last,
                                                             sizeof(last),
                                                             &create_rev),
                        CETCD_OK);
    ...
    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_create_rev(fixed64, 1,
                                                             &create_rev),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_create_rev(buf, n, NULL),
                        CETCD_ERR_INVAL);
}

CETCD_TEST_CASE(auto_compact_parse_watch_event_kv_mod_rev) {
    uint8_t buf[16];
    size_t n = 0;
    int64_t mod_rev = 9;

    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_kv_mod_rev(0, buf, sizeof(buf),
                                                           &n),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(n == 0);
    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_kv_mod_rev(5, buf, sizeof(buf),
                                                           &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_mod_rev(buf, n, &mod_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(mod_rev == 5);

    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_mod_rev(NULL, 0, &mod_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(mod_rev == 0);
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_mod_rev(dummy, 1, &mod_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(mod_rev == 0);

    /* leftover truncated Event-nested mod_revision cannot look like 0 */
    uint8_t trunc[] = { 0x5a, 0x03, 0x12, 0x01, 0x18 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_mod_rev(trunc, sizeof(trunc),
                                                          &mod_rev),
                        CETCD_ERR_INVAL);

    /* leftover cannot steal a printed Event mod_revision */
    uint8_t steal[] = { 0x5a, 0x06, 0x12, 0x04, 0x32, 0x02, 0x18, 0x05 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_mod_rev(steal, sizeof(steal),
                                                          &mod_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(mod_rev == 0);

    /* leftover header revision is not Event mod_revision */
    uint8_t header[] = { 0x0a, 0x02, 0x18, 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_mod_rev(header,
                                                          sizeof(header),
                                                          &mod_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(mod_rev == 0);

    uint8_t last[] = { 0x5a, 0x04, 0x12, 0x02, 0x18, 0x01,
                       0x5a, 0x04, 0x12, 0x02, 0x18, 0x02 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_mod_rev(last, sizeof(last),
                                                          &mod_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(mod_rev == 2);

    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_mod_rev(fixed64, 1, &mod_rev),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_mod_rev(buf, n, NULL),
                        CETCD_ERR_INVAL);
}

CETCD_TEST_CASE(auto_compact_parse_watch_event_kv_value) {
    uint8_t buf[32];
    size_t n = 0;
    char value[16];

    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_kv_value("", buf, sizeof(buf),
                                                         &n),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(n == 0);
    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_kv_value("ok", buf, sizeof(buf),
                                                         &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_value(buf, n, value,
                                                        sizeof(value)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(value, "ok");

    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_value(NULL, 0, value,
                                                        sizeof(value)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(value, "");
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_value(dummy, 1, value,
                                                        sizeof(value)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(value, "");

    /* leftover truncated Event-nested value cannot print leftover text */
    uint8_t trunc[] = { 0x5a, 0x03, 0x12, 0x01, 0x2a };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_value(trunc, sizeof(trunc),
                                                        value, sizeof(value)),
                        CETCD_ERR_INVAL);

    /* leftover cannot steal a printed Event value */
    uint8_t steal[] = { 0x5a, 0x08, 0x12, 0x06, 0x32, 0x04, 0x2a, 0x02, 'o',
                        'k' };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_value(steal, sizeof(steal),
                                                        value, sizeof(value)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(value, "");

    /* leftover header is not Event value */
    uint8_t header[] = { 0x0a, 0x04, 0x2a, 0x02, 'x', 'y' };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_value(header, sizeof(header),
                                                        value, sizeof(value)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(value, "");

    uint8_t last[] = { 0x5a, 0x06, 0x12, 0x04, 0x2a, 0x02, 'a', 'a',
                       0x5a, 0x06, 0x12, 0x04, 0x2a, 0x02, 'b', 'b' };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_value(last, sizeof(last),
                                                        value, sizeof(value)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(value, "bb");

    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_value(fixed64, 1, value,
                                                        sizeof(value)),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_kv_value(buf, n, NULL, 0),
                        CETCD_ERR_INVAL);
}
...
CETCD_TEST_CASE(auto_compact_parse_watch_event_type) {
    uint8_t buf[16];
    size_t n = 0;
    int type = 9;

    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_type(-1, buf, sizeof(buf),
                                                     &n),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_type(0, buf, sizeof(buf),
                                                     &n),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(n == 0);
    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_type(1, buf, sizeof(buf),
                                                     &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_type(buf, n, &type),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(type, 1);

    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_type(NULL, 0, &type),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(type, 0);
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_type(dummy, 1, &type),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(type, 0);

    /* leftover truncated Event type cannot look like PUT */
    uint8_t trunc[] = { 0x5a, 0x01, 0x08 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_type(trunc, sizeof(trunc),
                                                     &type),
                        CETCD_ERR_INVAL);

    /* leftover cannot steal a printed Event type */
    uint8_t steal[] = { 0x5a, 0x04, 0x32, 0x02, 0x08, 0x01 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_type(steal, sizeof(steal),
                                                     &type),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(type, 0);

    /* leftover header cluster_id is not Event type */
    uint8_t header[] = { 0x0a, 0x02, 0x08, 0x01 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_type(header, sizeof(header),
                                                     &type),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(type, 0);

    uint8_t last[] = { 0x5a, 0x02, 0x08, 0x00, 0x5a, 0x02, 0x08, 0x01 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_type(last, sizeof(last),
                                                     &type),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(type, 1);

    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_type(fixed64, 1, &type),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_type(buf, n, NULL),
                        CETCD_ERR_INVAL);
}
...
CETCD_TEST_CASE(auto_compact_parse_watch_event_prev_kv_mod_rev) {
    uint8_t buf[16];
    size_t n = 0;
    int64_t mod_rev = 9;

    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_prev_kv_mod_rev(0, buf,
                                                                sizeof(buf),
                                                                &n),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(n == 0);
    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_prev_kv_mod_rev(5, buf,
                                                                sizeof(buf),
                                                                &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_mod_rev(buf, n,
                                                               &mod_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(mod_rev == 5);

    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_mod_rev(NULL, 0,
                                                               &mod_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(mod_rev == 0);
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_mod_rev(dummy, 1,
                                                               &mod_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(mod_rev == 0);

    /* leftover truncated Event-nested prev mod_revision cannot look like 0 */
    uint8_t trunc[] = { 0x5a, 0x03, 0x1a, 0x01, 0x18 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_mod_rev(trunc,
                                                               sizeof(trunc),
                                                               &mod_rev),
                        CETCD_ERR_INVAL);

    /* leftover cannot steal a printed Event prev mod_revision */
    uint8_t steal[] = { 0x5a, 0x06, 0x1a, 0x04, 0x32, 0x02, 0x18, 0x05 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_mod_rev(steal,
                                                               sizeof(steal),
                                                               &mod_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(mod_rev == 0);

    /* leftover header revision is not Event prev mod_revision */
    uint8_t header[] = { 0x0a, 0x02, 0x18, 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_mod_rev(header,
                                                               sizeof(header),
                                                               &mod_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(mod_rev == 0);

    uint8_t last[] = { 0x5a, 0x04, 0x1a, 0x02, 0x18, 0x01,
                       0x5a, 0x04, 0x1a, 0x02, 0x18, 0x02 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_mod_rev(last,
                                                               sizeof(last),
                                                               &mod_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(mod_rev == 2);
    ...
    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_mod_rev(fixed64, 1,
                                                               &mod_rev),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_mod_rev(buf, n, NULL),
                        CETCD_ERR_INVAL);
}

CETCD_TEST_CASE(auto_compact_parse_watch_event_prev_kv_version) {
    uint8_t buf[16];
    size_t n = 0;
    int64_t version = 9;

    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_prev_kv_version(0, buf,
                                                                sizeof(buf),
                                                                &n),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(n == 0);
    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_prev_kv_version(3, buf,
                                                                sizeof(buf),
                                                                &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_version(buf, n,
                                                               &version),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(version == 3);

    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_version(NULL, 0,
                                                               &version),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(version == 0);
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_version(dummy, 1,
                                                               &version),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(version == 0);

    /* leftover truncated Event-nested prev version cannot look like 0 */
    uint8_t trunc[] = { 0x5a, 0x03, 0x1a, 0x01, 0x20 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_version(trunc,
                                                               sizeof(trunc),
                                                               &version),
                        CETCD_ERR_INVAL);

    /* leftover cannot steal a printed Event prev version */
    uint8_t steal[] = { 0x5a, 0x06, 0x1a, 0x04, 0x32, 0x02, 0x20, 0x03 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_version(steal,
                                                               sizeof(steal),
                                                               &version),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(version == 0);

    /* leftover header raft_term is not Event prev version */
    uint8_t header[] = { 0x0a, 0x02, 0x20, 0x07 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_version(header,
                                                               sizeof(header),
                                                               &version),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(version == 0);

    uint8_t last[] = { 0x5a, 0x04, 0x1a, 0x02, 0x20, 0x01,
                       0x5a, 0x04, 0x1a, 0x02, 0x20, 0x02 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_version(last,
                                                               sizeof(last),
                                                               &version),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(version == 2);
    ...
    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_version(fixed64, 1,
                                                               &version),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_version(buf, n, NULL),
                        CETCD_ERR_INVAL);
}

CETCD_TEST_CASE(auto_compact_parse_watch_event_prev_kv_key) {
    uint8_t buf[32];
    size_t n = 0;
    char key[16];

    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_prev_kv_key("", buf,
                                                            sizeof(buf), &n),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(n == 0);
    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_prev_kv_key("/p", buf,
                                                            sizeof(buf), &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_key(buf, n, key,
                                                           sizeof(key)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(key, "/p");

    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_key(NULL, 0, key,
                                                           sizeof(key)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(key, "");
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_key(dummy, 1, key,
                                                           sizeof(key)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(key, "");

    /* leftover truncated Event-nested prev key cannot print leftover text */
    uint8_t trunc[] = { 0x5a, 0x03, 0x1a, 0x01, 0x0a };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_key(trunc, sizeof(trunc),
                                                           key, sizeof(key)),
                        CETCD_ERR_INVAL);

    /* leftover cannot steal a printed Event prev key */
    uint8_t steal[] = { 0x5a, 0x08, 0x1a, 0x06, 0x32, 0x04, 0x0a, 0x02, '/',
                        'p' };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_key(steal, sizeof(steal),
                                                           key, sizeof(key)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(key, "");

    /* leftover header is not Event prev key */
    uint8_t header[] = { 0x0a, 0x04, 0x0a, 0x02, 'x', 'y' };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_key(header,
                                                           sizeof(header),
                                                           key, sizeof(key)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(key, "");

    uint8_t last[] = { 0x5a, 0x06, 0x1a, 0x04, 0x0a, 0x02, '/', 'a',
                       0x5a, 0x06, 0x1a, 0x04, 0x0a, 0x02, '/', 'b' };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_key(last, sizeof(last),
                                                           key, sizeof(key)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(key, "/b");

    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_key(fixed64, 1, key,
                                                           sizeof(key)),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_key(buf, n, NULL, 0),
                        CETCD_ERR_INVAL);
}

CETCD_TEST_CASE(auto_compact_parse_watch_event_prev_kv_value) {
    uint8_t buf[32];
    size_t n = 0;
    char value[16];

    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_prev_kv_value("", buf,
                                                              sizeof(buf),
                                                              &n),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(n == 0);
    CETCD_ASSERT_EQ_INT(cetcd_encode_watch_event_prev_kv_value("ok", buf,
                                                              sizeof(buf),
                                                              &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_value(buf, n, value,
                                                             sizeof(value)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(value, "ok");

    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_value(NULL, 0, value,
                                                             sizeof(value)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(value, "");
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_value(dummy, 1, value,
                                                             sizeof(value)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(value, "");

    /* leftover truncated Event-nested prev value cannot print leftover text */
    uint8_t trunc[] = { 0x5a, 0x03, 0x1a, 0x01, 0x2a };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_value(trunc,
                                                             sizeof(trunc),
                                                             value,
                                                             sizeof(value)),
                        CETCD_ERR_INVAL);

    /* leftover cannot steal a printed Event prev value */
    uint8_t steal[] = { 0x5a, 0x08, 0x1a, 0x06, 0x32, 0x04, 0x2a, 0x02, 'o',
                        'k' };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_value(steal,
                                                             sizeof(steal),
                                                             value,
                                                             sizeof(value)),
                        CETCD_OK);
    ...
    /* leftover header is not Event prev value */
    uint8_t header[] = { 0x0a, 0x04, 0x2a, 0x02, 'x', 'y' };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_value(header,
                                                             sizeof(header),
                                                             value,
                                                             sizeof(value)),
                        CETCD_OK);
    ...
    uint8_t last[] = { 0x5a, 0x06, 0x1a, 0x04, 0x2a, 0x02, 'a', 'a',
                       0x5a, 0x06, 0x1a, 0x04, 0x2a, 0x02, 'b', 'b' };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_value(last, sizeof(last),
                                                             value,
                                                             sizeof(value)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(value, "bb");
    ...
    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_value(fixed64, 1, value,
                                                             sizeof(value)),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_watch_event_prev_kv_value(buf, n, NULL, 0),
                        CETCD_ERR_INVAL);
}
...
CETCD_TEST_CASE(auto_compact_parse_delete_range_response) {
    ...
    CETCD_ASSERT_EQ_INT(cetcd_encode_delete_range_prev_kv(NULL, buf,
                                                          sizeof(buf), &n),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_encode_delete_range_prev_kv("/d", buf,
                                                          sizeof(buf), &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_delete_range_response(buf, n, key,
                                                          sizeof(key), &got),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(key, "/d");
    CETCD_ASSERT_TRUE(got == 1);

    CETCD_ASSERT_EQ_INT(cetcd_parse_delete_range_response(NULL, 0, key,
                                                          sizeof(key), &got),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(key, "");
    CETCD_ASSERT_TRUE(got == 0);
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_delete_range_response(dummy, 1, key,
                                                          sizeof(key), &got),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(got == 0);

    /* leftover truncated prev_kv cannot print a leftover key */
    uint8_t trunc[] = { 0x1a };
    CETCD_ASSERT_EQ_INT(cetcd_parse_delete_range_response(trunc, 1, key,
                                                          sizeof(key), &got),
                        CETCD_ERR_INVAL);
    uint8_t trunc_key[] = { 0x1a, 0x01, 0x0a };
    CETCD_ASSERT_EQ_INT(cetcd_parse_delete_range_response(trunc_key,
                                                          sizeof(trunc_key),
                                                          key, sizeof(key),
                                                          &got),
                        CETCD_ERR_INVAL);

    /* leftover cannot steal a printed delete key */
    uint8_t steal[] = { 0x22, 0x06, 0x1a, 0x04, 0x0a, 0x02, '/', 'd' };
    CETCD_ASSERT_EQ_INT(cetcd_parse_delete_range_response(steal, sizeof(steal),
                                                          key, sizeof(key),
                                                          &got),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(got == 0);
    ...
    uint8_t inner_steal[] = { 0x1a, 0x06, 0x12, 0x04, 0x0a, 0x02, '/', 'd' };
    CETCD_ASSERT_EQ_INT(cetcd_parse_delete_range_response(inner_steal,
                                                          sizeof(inner_steal),
                                                          key, sizeof(key),
                                                          &got),
                        CETCD_OK);
    ...
    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_delete_range_response(fixed64, 1, key,
                                                          sizeof(key), &got),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_delete_range_response(buf, n, NULL, 0,
                                                          NULL),
                        CETCD_ERR_INVAL);
}
...
CETCD_TEST_CASE(auto_compact_parse_delete_range_prev_kv_version) {
    uint8_t buf[16];
    size_t n = 0;
    int64_t version = 9;

    CETCD_ASSERT_EQ_INT(cetcd_encode_delete_range_prev_kv_version(0, buf,
                                                                 sizeof(buf),
                                                                 &n),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(n == 0);
    CETCD_ASSERT_EQ_INT(cetcd_encode_delete_range_prev_kv_version(4, buf,
                                                                 sizeof(buf),
                                                                 &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_delete_range_prev_kv_version(buf, n,
                                                                &version),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(version == 4);

    CETCD_ASSERT_EQ_INT(cetcd_parse_delete_range_prev_kv_version(NULL, 0,
                                                                &version),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(version == 0);
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_delete_range_prev_kv_version(dummy, 1,
                                                                &version),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(version == 0);

    /* leftover truncated prev version cannot look like 0 */
    uint8_t trunc[] = { 0x1a, 0x01, 0x20 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_delete_range_prev_kv_version(trunc,
                                                                sizeof(trunc),
                                                                &version),
                        CETCD_ERR_INVAL);

    /* leftover cannot steal a printed prev version */
    uint8_t steal[] = { 0x1a, 0x04, 0x32, 0x02, 0x20, 0x04 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_delete_range_prev_kv_version(steal,
                                                                sizeof(steal),
                                                                &version),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(version == 0);

    /* leftover header raft_term is not prev version */
    uint8_t header[] = { 0x0a, 0x02, 0x20, 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_delete_range_prev_kv_version(header,
                                                                sizeof(header),
                                                                &version),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(version == 0);

    uint8_t last[] = { 0x1a, 0x02, 0x20, 0x01, 0x1a, 0x02, 0x20, 0x02 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_delete_range_prev_kv_version(last,
                                                                sizeof(last),
                                                                &version),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(version == 2);
    ...
    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_delete_range_prev_kv_version(fixed64, 1,
                                                                &version),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_delete_range_prev_kv_version(buf, n, NULL),
                        CETCD_ERR_INVAL);
}
...
CETCD_TEST_CASE(auto_compact_parse_alarm_response) {
    uint8_t buf[32];
    size_t n = 0, got = 9;
    cetcd_alarm_member al[2];

    CETCD_ASSERT_EQ_INT(cetcd_encode_alarm_response_member(7, -1, buf,
                                                          sizeof(buf), &n),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_encode_alarm_response_member(7, 2, buf,
                                                          sizeof(buf), &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_alarm_response(buf, n, al, 2, &got),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(got == 1);
    CETCD_ASSERT_TRUE(al[0].member_id == 7);
    CETCD_ASSERT_EQ_INT(al[0].alarm, 2);

    CETCD_ASSERT_EQ_INT(cetcd_parse_alarm_response(NULL, 0, al, 2, &got),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(got == 0);
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_alarm_response(dummy, 1, al, 2, &got),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(got == 0);

    /* leftover truncated alarm cannot look like NONE */
    uint8_t trunc[] = { 0x12, 0x01, 0x10 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_alarm_response(trunc, sizeof(trunc), al, 2,
                                                   &got),
                        CETCD_ERR_INVAL);

    /* leftover cannot steal CORRUPT */
    uint8_t steal[] = { 0x1a, 0x04, 0x12, 0x02, 0x10, 0x02 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_alarm_response(steal, sizeof(steal), al, 2,
                                                   &got),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(got == 0);

    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_alarm_response(fixed64, 1, al, 2, &got),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_alarm_response(buf, n, NULL, 1, &got),
                        CETCD_ERR_INVAL);
}

CETCD_TEST_CASE(auto_compact_parse_range_response_kv) {
    uint8_t buf[64];
    size_t n = 0, nk = 9;
    char key[32], val[32];

    CETCD_ASSERT_EQ_INT(cetcd_encode_range_response_kv(NULL, 0, NULL, 0, buf,
                                                       sizeof(buf), &n),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_encode_range_response_kv((const uint8_t *)"foo",
                                                       3, (const uint8_t *)"v",
                                                       1, buf, sizeof(buf),
                                                       &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv(buf, n, key, sizeof(key),
                                                      val, sizeof(val), &nk),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(nk == 1);
    CETCD_ASSERT_EQ_STR(key, "foo");
    CETCD_ASSERT_EQ_STR(val, "v");

    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv(NULL, 0, key, sizeof(key),
                                                      val, sizeof(val), &nk),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(nk == 0);
    CETCD_ASSERT_EQ_STR(key, "");
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv(dummy, 1, key,
                                                      sizeof(key), val,
                                                      sizeof(val), &nk),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(nk == 0);

    /* leftover truncated key cannot print a leftover key */
    uint8_t trunc[] = { 0x12, 0x01, 0x0a };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv(trunc, sizeof(trunc),
                                                      key, sizeof(key), val,
                                                      sizeof(val), &nk),
                        CETCD_ERR_INVAL);
    ...
    /* leftover cannot steal a printed key */
    uint8_t steal[] = {
        0x22, 0x07, 0x12, 0x05, 0x0a, 0x03, 'f', 'o', 'o'
    };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv(steal, sizeof(steal),
                                                      key, sizeof(key), val,
                                                      sizeof(val), &nk),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(nk == 0);
    ...
    uint8_t steal_key[] = {
        0x12, 0x07, 0x3a, 0x05, 0x0a, 0x03, 'f', 'o', 'o'
    };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv(steal_key,
                                                      sizeof(steal_key), key,
                                                      sizeof(key), val,
                                                      sizeof(val), &nk),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(nk == 1);
    ...
    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv(fixed64, 1, key,
                                                      sizeof(key), val,
                                                      sizeof(val), &nk),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv(buf, n, NULL, 0, NULL, 0,
                                                      NULL),
                        CETCD_ERR_INVAL);
}

CETCD_TEST_CASE(auto_compact_parse_range_response_kv_lease) {
    uint8_t buf[16];
    size_t n = 0;
    int64_t lease = 9;

    CETCD_ASSERT_EQ_INT(cetcd_encode_range_response_kv_lease(0, buf,
                                                            sizeof(buf), &n),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(n == 0);
    CETCD_ASSERT_EQ_INT(cetcd_encode_range_response_kv_lease(5, buf,
                                                            sizeof(buf), &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_lease(buf, n, &lease),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(lease == 5);

    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_lease(NULL, 0, &lease),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(lease == 0);
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_lease(dummy, 1, &lease),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(lease == 0);

    /* leftover truncated lease cannot look like lease 0 */
    uint8_t trunc[] = { 0x12, 0x01, 0x30 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_lease(trunc, sizeof(trunc),
                                                           &lease),
                        CETCD_ERR_INVAL);

    /* leftover cannot steal a printed lease */
    uint8_t steal[] = { 0x12, 0x04, 0x32, 0x02, 0x30, 0x05 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_lease(steal, sizeof(steal),
                                                           &lease),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(lease == 0);

    /* leftover header is not lease */
    uint8_t header[] = { 0x0a, 0x02, 0x30, 0x05 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_lease(header,
                                                           sizeof(header),
                                                           &lease),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(lease == 0);

    uint8_t last[] = { 0x12, 0x02, 0x30, 0x01, 0x12, 0x02, 0x30, 0x02 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_lease(last, sizeof(last),
                                                           &lease),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(lease == 2);

    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_lease(fixed64, 1, &lease),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_lease(buf, n, NULL),
                        CETCD_ERR_INVAL);
}
...
CETCD_TEST_CASE(auto_compact_parse_range_response_kv_mod_rev) {
    uint8_t buf[16];
    size_t n = 0;
    int64_t mod_rev = 9;

    CETCD_ASSERT_EQ_INT(cetcd_encode_range_response_kv_mod_rev(0, buf,
                                                              sizeof(buf),
                                                              &n),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(n == 0);
    CETCD_ASSERT_EQ_INT(cetcd_encode_range_response_kv_mod_rev(5, buf,
                                                              sizeof(buf),
                                                              &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_mod_rev(buf, n,
                                                             &mod_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(mod_rev == 5);

    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_mod_rev(NULL, 0,
                                                             &mod_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(mod_rev == 0);
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_mod_rev(dummy, 1,
                                                             &mod_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(mod_rev == 0);

    /* leftover truncated mod_revision cannot look like 0 */
    uint8_t trunc[] = { 0x12, 0x01, 0x18 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_mod_rev(trunc,
                                                             sizeof(trunc),
                                                             &mod_rev),
                        CETCD_ERR_INVAL);

    /* leftover cannot steal a printed mod_revision */
    uint8_t steal[] = { 0x12, 0x04, 0x32, 0x02, 0x18, 0x05 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_mod_rev(steal,
                                                             sizeof(steal),
                                                             &mod_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(mod_rev == 0);

    /* leftover header revision is not mod_revision */
    uint8_t header[] = { 0x0a, 0x02, 0x18, 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_mod_rev(header,
                                                             sizeof(header),
                                                             &mod_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(mod_rev == 0);

    uint8_t last[] = { 0x12, 0x02, 0x18, 0x01, 0x12, 0x02, 0x18, 0x02 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_mod_rev(last,
                                                             sizeof(last),
                                                             &mod_rev),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(mod_rev == 2);
    ...
    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_mod_rev(fixed64, 1,
                                                             &mod_rev),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_mod_rev(buf, n, NULL),
                        CETCD_ERR_INVAL);
}
...
CETCD_TEST_CASE(auto_compact_parse_range_response_kv_key) {
    uint8_t buf[32];
    size_t n = 0;
    char key[16];

    CETCD_ASSERT_EQ_INT(cetcd_encode_range_response_kv_key("", buf, sizeof(buf),
                                                          &n),
                        CETCD_OK);
    CETCD_ASSERT_TRUE(n == 0);
    CETCD_ASSERT_EQ_INT(cetcd_encode_range_response_kv_key("/k", buf, sizeof(buf),
                                                          &n),
                        CETCD_OK);
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_key(buf, n, key,
                                                         sizeof(key)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(key, "/k");

    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_key(NULL, 0, key,
                                                         sizeof(key)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(key, "");
    uint8_t dummy[] = { 0x00 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_key(dummy, 1, key,
                                                         sizeof(key)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(key, "");
    ...
    /* leftover truncated key cannot print leftover text */
    uint8_t trunc[] = { 0x12, 0x01, 0x0a };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_key(trunc, sizeof(trunc),
                                                         key, sizeof(key)),
                        CETCD_ERR_INVAL);

    /* leftover cannot steal a printed key */
    uint8_t steal[] = { 0x12, 0x06, 0x32, 0x04, 0x0a, 0x02, '/', 'k' };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_key(steal, sizeof(steal),
                                                         key, sizeof(key)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(key, "");

    /* leftover header is not key */
    uint8_t header[] = { 0x0a, 0x04, 0x0a, 0x02, '/', 'h' };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_key(header, sizeof(header),
                                                         key, sizeof(key)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(key, "");

    uint8_t last[] = { 0x12, 0x04, 0x0a, 0x02, '/', 'a',
                       0x12, 0x04, 0x0a, 0x02, '/', 'b' };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_key(last, sizeof(last),
                                                         key, sizeof(key)),
                        CETCD_OK);
    CETCD_ASSERT_EQ_STR(key, "/b");

    uint8_t fixed64[] = { 0x09 };
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_key(fixed64, 1, key,
                                                         sizeof(key)),
                        CETCD_ERR_INVAL);
    CETCD_ASSERT_EQ_INT(cetcd_parse_range_response_kv_key(buf, n, NULL, 0),
                        CETCD_ERR_INVAL);
}
...
CETCD_TEST_LIST_BEGIN
    ...
    CETCD_TEST_ENTRY(auto_compact_parse_status_is_learner),
    ...
    CETCD_TEST_ENTRY(auto_compact_parse_status_raft_applied),
    ...
    CETCD_TEST_ENTRY(auto_compact_parse_watch_event_type),
    ...
    CETCD_TEST_ENTRY(auto_compact_parse_watch_event_prev_kv_key),
    CETCD_TEST_ENTRY(auto_compact_parse_watch_event_prev_kv_value),
    ...
    CETCD_TEST_ENTRY(auto_compact_parse_range_response_kv_key),
