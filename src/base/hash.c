...
static atomic_int crc32c_table_inited = 0;
...
static void crc32c_init_table(void) {
    ...
    atomic_store_explicit(&crc32c_table_inited, 1, memory_order_release);
}

uint32_t cetcd_crc32c(uint32_t crc, const void *data, size_t n) {
    if (atomic_load_explicit(&crc32c_table_inited, memory_order_acquire) == 0)
        crc32c_init_table();
    const uint8_t *p = (const uint8_t *)data;
