...
static int tls_peer_identity_ok_(cetcd_server *srv, cetcd_tls_conn *tls) {
    if (!srv || !tls) return 0;
    if (cetcd_tls_check_peer_identity(tls, srv->cfg.peer_cert_allowed_cn,
                                      srv->cfg.peer_cert_allowed_hostname) !=
        CETCD_OK)
        return 0;
    return cetcd_tls_check_crl(tls, srv->tls_peer) == CETCD_OK;
}
static int tls_client_identity_ok_(cetcd_server *srv, cetcd_tls_conn *tls) {
    if (!srv || !tls) return 0;
    if (cetcd_tls_check_peer_identity(tls, NULL,
                                      srv->cfg.client_cert_allowed_hostname) !=
        CETCD_OK)
        return 0;
    return cetcd_tls_check_crl(tls, srv->tls_client) == CETCD_OK;
}
...
static void on_peer_tx_read_(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf) {
    ...
    if (!tx->tls_ready) {
        ...
        if (hs == 0) return;
        if (!tls_peer_identity_ok_(tx->srv, tx->tls)) {
            peer_tx_close_(tx);
            return;
        }
        tx->tls_ready = 1;
...
static void on_peer_read_(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf) {
    ...
    if (ctx->tls) {
        ...
        if (!ctx->tls_ready) {
            ...
            if (hs == 0) return;
            if (!tls_peer_identity_ok_(ctx->srv, ctx->tls)) {
                uv_close((uv_handle_t *)stream, on_peer_close_);
                return;
            }
            ctx->tls_ready = 1;
...
static int load_tls_ctx_(cetcd_tls_ctx **out,
    ...
    if (ca && ca[0]) {
        ...
        if (crl && crl[0]) {
            int crc = cetcd_tls_set_crl(ctx, crl);
            if (crc != CETCD_OK) {
                cetcd_tls_ctx_free(ctx);
                return crc;
            }
...
int cetcd_server_start(cetcd_server *srv) {
    ...
    cetcd_v3rpc_set_strict_reconfig(srv->cfg.strict_reconfig_set
                                        ? (srv->cfg.strict_reconfig ? 1 : 0)
                                        : 1);
    ...
    if (cetcd_tls_crl_requires_cert(srv->cfg.client_crl_file,
                                    srv->cfg.cert_file) != CETCD_OK)
        return CETCD_ERR_INVAL;
    if (cetcd_tls_crl_requires_cert(srv->cfg.peer_crl_file,
                                    srv->cfg.peer_cert_file) != CETCD_OK)
        return CETCD_ERR_INVAL;
    if (srv->cfg.cipher_suites[0] &&
        !(srv->cfg.cert_file[0] || srv->cfg.peer_cert_file[0]))
        return CETCD_ERR_INVAL;
    if ((!cetcd_tls_name_list_open(srv->cfg.peer_cert_allowed_cn) ||
         !cetcd_tls_name_list_open(srv->cfg.peer_cert_allowed_hostname)) &&
        !(srv->cfg.peer_cert_file[0] && srv->cfg.peer_trusted_ca_file[0]))
        return CETCD_ERR_INVAL;
    if (!cetcd_tls_name_list_open(srv->cfg.client_cert_allowed_hostname) &&
        !(srv->cfg.cert_file[0] && srv->cfg.trusted_ca_file[0]))
        return CETCD_ERR_INVAL;
    ...
    if (!srv->tls_client && !srv->tls_peer && !srv->tls_peer_out) {
        ...
        trc = load_tls_ctx_(&srv->tls_peer,
                            ...
                            srv->cfg.peer_trusted_ca_file, peer_auth, 0,
                            srv->cfg.cipher_suites, vmin, vmax,
