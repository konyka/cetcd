...
`--experimental-initial-corrupt-check` hashes the store after WAL replay and compares `{data-dir}/backend.hash` (mismatch or a lower current revision fail-closes). `--experimental-corrupt-check-time` repeats that HashKV on the tick (`0` disables; mismatch raises CORRUPT; a non-duration fail-closes). `--experimental-compaction-batch-limit` caps auto-compact to N revisions per tick (`0` unlimited; leftover text fail-closes). `--experimental-watch-progress-notify-interval` sets Watch `progress_notify` (`0` = 10s; a non-duration fail-closes). `--experimental-warning-apply-duration` warns if apply is slower (`0` disables; omitted default 100ms). `--experimental-max-learners` caps learner `MemberAdd` (`0` = none; omitted default 1; leftover text fail-closes). Other `--experimental-*` stay no-ops.
