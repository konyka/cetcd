...
RoleGet leftover-safe-parses so leftover length-delimited bytes cannot steal a printed Permission key, permType, or range_end (truncated perm / key / range_end fail-closes). RoleGet leftover-safe-parses permType so leftover length-delimited bytes cannot steal a printed permType (truncated permType fail-closes; tag `0x0a` header cluster_id is not permType). UserGet / UserList / RoleList leftover-safe-parse first so leftover cannot steal a printed name (truncated name fail-closes). Permission leftover-safe-encodes etcd field 2 `key` / field 3 `range_end`.
