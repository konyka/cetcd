...
AlarmResponse leftover-safe-parses so leftover length-delimited bytes cannot steal a printed memberID or CORRUPT type. RangeResponse leftover-safe-parses so leftover cannot steal a printed `get` / `txn` key.
