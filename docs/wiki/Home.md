...
LeaseGrant / TimeToLive leftover-safe-parses so leftover length-delimited bytes cannot steal a printed or lock-used ID or key (tag `0x08` is not the grant ID; truncated ID / key fail-closes). LeaseGrant leftover-safe-parses field 4 so leftover cannot steal a printed grant error (truncated error fail-closes; cetcd omits field 4 on success). TimeToLive leftover-safe-parses field 5 so leftover cannot steal a printed TTL key (truncated key fail-closes). TimeToLive leftover-safe-parses field 4 so leftover cannot steal a printed grantedTTL (truncated grantedTTL fail-closes; tag `0x0a` header raft_term is not grantedTTL). TimeToLive leftover-safe-parses field 3 so leftover cannot steal a printed remaining TTL (truncated remaining TTL fail-closes; tag `0x0a` header revision is not remaining TTL). TimeToLive leftover-safe-parses field 2 so leftover cannot steal a printed lease ID (truncated ID fail-closes; tag `0x0a` header member_id is not the lease ID).
`cetcdctl lease grant` / `timetolive` leftover-safe-parse json/fields so leftover cannot steal a printed ID, TTL key, or grant error.
...
LeaseLeases leftover-safe-parses so leftover length-delimited bytes cannot steal a printed lease ID (truncated lease / ID fail-closes). `cetcdctl lease list` leftover-safe-parses field 1 first.
...
Snapshot leftover-safe-parses so leftover length-delimited bytes cannot steal the blob written to disk (truncated blob fail-closes).
...
AlarmResponse leftover-safe-parses so leftover length-delimited bytes cannot steal a printed memberID or CORRUPT type. RangeResponse leftover-safe-parses so leftover cannot steal a printed `get` / `txn` key or a printed `get --count-only` count / `more=true` (truncated count / more fail-closes).
...
Downgrade leftover-safe-parses so leftover length-delimited bytes cannot steal a printed cluster version (truncated version fail-closes).
...
Range leftover-safe-parses KV lease so leftover length-delimited bytes cannot steal a printed lease (truncated lease fail-closes; tag `0x0a` header is not lease).
...
Range leftover-safe-parses KV key so leftover length-delimited bytes cannot steal a printed key (truncated key fail-closes; tag `0x0a` header is not key).
...
DeleteRange leftover-safe-parses so leftover length-delimited bytes cannot steal a printed prev_kv key (truncated prev_kv / key fail-closes; tag `0x0a` header is not prev key).
...
Txn leftover-safe-parses `succeeded` so leftover length-delimited bytes cannot steal a successful lock, elect, or compare (truncated succeeded fail-closes). `cetcdctl txn del` / `txn get` leftover-safe-parse field 2 first so a leftover truncated succeeded cannot print true or range kvs.
...
AuthStatus leftover-safe-parses so leftover length-delimited bytes cannot steal `enabled`. AuthStatus leftover-safe-parses authRevision so leftover length-delimited bytes cannot steal a printed authRevision (truncated authRevision fail-closes; tag `0x0a` header revision is not authRevision). UserList/RoleList leftover-safe-parses so leftover cannot steal a printed user or role.
RoleGet leftover-safe-parses so leftover length-delimited bytes cannot steal a printed Permission key, permType, or range_end (truncated perm / key / range_end fail-closes). RoleGet leftover-safe-parses permType so leftover length-delimited bytes cannot steal a printed permType (truncated permType fail-closes; tag `0x0a` header cluster_id is not permType). RoleGet leftover-safe-parses perm key so leftover length-delimited bytes cannot steal a printed perm key (truncated perm key fail-closes; tag `0x0a` header is not perm key). UserGet / UserList / RoleList leftover-safe-parse first so leftover cannot steal a printed name (truncated name fail-closes). Permission leftover-safe-encodes etcd field 2 `key` / field 3 `range_end`.
Status leftover-safe-parses so leftover length-delimited bytes cannot steal a printed version, dbSize, dbSizeInUse, leader, raftIndex, raftTerm, raftAppliedIndex, `isLearner`, or alarm error (truncated version / isLearner / dbSize / dbSizeInUse / leader / raftIndex / raftTerm / raftAppliedIndex / error fail-closes; tag `0x0a` header is not version or dbSize).
...
Watch leftover-safe-parses Event KV lease so leftover length-delimited bytes cannot steal a printed Event lease (truncated Event-nested lease fail-closes; tag `0x0a` header raft_term is not Event lease).
Watch leftover-safe-parses Event KV version so leftover length-delimited bytes cannot steal a printed Event version (truncated Event-nested version fail-closes; tag `0x0a` header raft_term is not Event version).
Watch leftover-safe-parses Event KV create_revision so leftover length-delimited bytes cannot steal a printed Event create_revision (truncated Event-nested create_revision fail-closes; tag `0x0a` header member_id is not Event create_revision).
Watch leftover-safe-parses Event KV mod_revision so leftover length-delimited bytes cannot steal a printed Event mod_revision (truncated Event-nested mod_revision fail-closes; tag `0x0a` header revision is not Event mod_revision).
Watch leftover-safe-parses Event KV value so leftover length-delimited bytes cannot steal a printed Event value (truncated Event-nested value fail-closes; tag `0x0a` header is not Event value).
Watch leftover-safe-parses Event KV key so leftover length-delimited bytes cannot steal a printed Event key (truncated Event-nested key fail-closes; tag `0x0a` header is not Event key).
Watch leftover-safe-parses Event type so leftover length-delimited bytes cannot steal a printed Event type (truncated Event type fail-closes; tag `0x0a` header cluster_id is not Event type).
Watch leftover-safe-parses Event prev_kv create_revision so leftover length-delimited bytes cannot steal a printed prev create_revision (truncated Event-nested prev create_revision fail-closes; tag `0x0a` header member_id is not prev create_revision).
Watch leftover-safe-parses Event prev_kv mod_revision so leftover length-delimited bytes cannot steal a printed prev mod_revision (truncated Event-nested prev mod_revision fail-closes; tag `0x0a` header revision is not prev mod_revision).
Watch leftover-safe-parses Event prev_kv version so leftover length-delimited bytes cannot steal a printed prev version (truncated Event-nested prev version fail-closes; tag `0x0a` header raft_term is not prev version).
Watch leftover-safe-parses Event prev_kv key so leftover length-delimited bytes cannot steal a printed prev key (truncated Event-nested prev key fail-closes; tag `0x0a` header is not prev key).
Watch leftover-safe-parses Event prev_kv value so leftover length-delimited bytes cannot steal a printed prev value (truncated Event-nested prev value fail-closes; tag `0x0a` header is not prev value).
...
Watch leftover-safe-parses `cancel_reason` so leftover length-delimited bytes cannot steal a printed compact cancel (truncated reason fail-closes). Compacted WatchCreate / active-watch cancel encodes etcd `ErrCompacted`; `cetcdctl watch` leftover-safe-parses field 6.
