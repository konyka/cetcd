...
Status leftover-safe-parses so leftover length-delimited bytes cannot steal a printed version, dbSize, dbSizeInUse, leader, raftIndex, `isLearner`, or alarm error (truncated version / isLearner / dbSizeInUse / leader / raftIndex / error fail-closes).
