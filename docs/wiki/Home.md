...
RoleGet leftover-safe-parses so leftover length-delimited bytes cannot steal a printed Permission key, permType, or range_end (truncated perm / key / range_end fail-closes). Permission leftover-safe-encodes etcd field 2 `key` / field 3 `range_end`.
