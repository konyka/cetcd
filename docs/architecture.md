...
`lease grant --lease-id ID` (custom lease ID in hex format, matching etcdctl behavior; a non-hex id fail-closes; grant TTL must be `> 0`; a typo fail-closes),
...
`member add --peer-urls url1,url2` (comma-separated multiple peer URLs for cluster member addition; `member update` also supports comma-separated URLs; peer URL ports are leftover-safe `1..65535`; `member remove`/`update`/`promote` IDs are hex `> 0`; leftover text fail-closes),
