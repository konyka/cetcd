...
| Raft | State machine + peer TCP framing; Put/DeleteRange propose, apply after WAL sync. Server `pre_vote` runs a PreVote round (no term bump) before a real election; `TIMEOUT_NOW` skips it. |
