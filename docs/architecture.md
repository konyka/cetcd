...
| Raft | State machine + peer TCP framing; Put/DeleteRange propose, apply after WAL sync. Server `pre_vote` runs a PreVote round (no term bump) before a real election (`--pre-vote=false` skips it; omitted default on); `TIMEOUT_NOW` skips it. Omitted `--initial-election-tick-advance` (default on) ticks `election_tick-1` at create so the first campaign is one tick away (`=false` waits the full timeout). `--heartbeat-interval` / `--election-timeout` (ms) set the timer period and `election_tick=election_ms/tick_ms` (cannot mix with `--*-tick`). |
...
| TLS | Memory-BIO termination on client/peer listen and on outbound `peer_tx_`. Client and peer listen select ALPN `h2` when offered. Cert without key, missing files, or `--client-cert-auth` without CA fail closed. `--trusted-ca-file` also requires a client certificate (etcd; `--client-cert-auth=false` does not opt out); `--peer-trusted-ca-file` does the same on peer accept. `--auto-tls` / `--peer-auto-tls` mint ECDSA P-256 into `{data-dir}/fixtures/` (`--self-signed-cert-validity` years, omitted default 1). `--tls-min-version` / `--tls-max-version` are `TLS1.2` or `TLS1.3` (omitted min TLS1.2; omitted max open; min > max fail-closes). `--peer-cert-allowed-cn` / `--peer-cert-allowed-hostname` / `--client-cert-allowed-hostname` close a handshake whose peer cert CN/SAN is not listed (restricted lists require cert+CA). `--peer-client-cert-file` / `--peer-client-key-file` present a distinct cert on outbound `peer_tx_` (omitted uses `--peer-cert-file`; cert without key or override without listen TLS fail-closes). `--client-crl-file` / `--peer-crl-file` reject a handshake whose peer cert serial is on the CRL (requires that side's cert; missing/garbage CRL fail-closes). `--version` prints and exits. `--config-file` applies an etcd YAML map of flag names through the same parser (other CLI flags and `ETCD_*` ignored). Without a config file, `ETCD_*` maps to `--flag` (empty ignored; `ETCD_VERSION` skipped; CLI+env conflict fail-closes). Plaintext remains the default. |
| Cluster join | `--initial-cluster-state existing` restarts from evidence or starts as a follower with `--initial-cluster` peers. `snapshot.kv` is imported into empty MVCC. Restore persists `--initial-cluster` / `--name` / advertise URLs into the data dir so start can omit them. Save writes CTS2 (revision + CRC32C); restore fail-closes on a hash mismatch unless `--skip-hash-check`. After WAL compaction the leader sends `MsgSnap` (KV blob) to a lagging joiner; while the log is still live it sends `App` from `next_idx`. `--auto-compaction-mode` / `--auto-compaction-retention` compact on the leader tick (0 disables; invalid values fail at parse). Linearizable Range fail-closes on a follower unless `serializable`. `Maintenance/Status` reports LMDB `dbSize` / `dbSizeInUse` (quota uses the same used-page size), `raftAppliedIndex`, alarm `errors`, and `isLearner`. `--experimental-initial-corrupt-check` compares HashKV to `{data-dir}/backend.hash` after WAL replay (mismatch or data-loss revision fail-closes). `--experimental-corrupt-check-time` repeats that HashKV on the tick (0 disables; mismatch raises CORRUPT). `--experimental-compaction-batch-limit` caps auto-compact to N revisions per tick (0 unlimited). `--experimental-compaction-sleep-interval` waits between those batches (0 = none). `--experimental-watch-progress-notify-interval` sets Watch progress_notify (0 = 10s). `--experimental-warning-apply-duration` warns if apply exceeds the duration (0 disables; omitted default 100ms). `--experimental-warning-unary-request-duration` warns if unary RPC exceeds the duration (0 disables; omitted default 300ms). `--experimental-max-learners` caps learner MemberAdd (0 = none; omitted default 1). `--experimental-memory-mlock` calls mlockall at start (Windows fail-closes). `--experimental-bootstrap-defrag-threshold-megabytes` compact-copies data.mdb at start if alloc exceeds N MiB (0 off). `Maintenance/Defragment` compact-copies when a backend is attached. Raft PreVote is on unless `--pre-vote=false` (stale or partitioned campaigns do not increment term until a majority grants). Omitted `--initial-election-tick-advance` ticks `election_tick-1` at Raft create (`=false` waits the full timeout). `--heartbeat-interval` / `--election-timeout` set the Raft timer in milliseconds (`election_tick=election_ms/tick_ms`; mix with `--*-tick` fail-closes). `MemberRemove` of a voter is fail-closed unless remaining voters still meet the old quorum (`--strict-reconfig-check=false` skips that gate). `Downgrade` VALIDATE of `cetcd_version()` succeeds; ENABLE/CANCEL and other versions fail-closed. Unimplemented or unknown `--experimental-*` fail at parse (`=false`/`0` is OK). |
...
(`123foo-456.snap` / decimal `atoll` cannot win latest), leftover-safe-parses
etcd `%016x-%016x.wal` so `123foo-456.wal` cannot be scanned and win latest, and leftover-safe-parses
`--data-dir` / `--output-dir` (`--flag=VALUE` or next argv; `--data-dir --output-dir`
cannot eat a flag as the path; empty `--flag=` fail-closes). A separate `etcd-snap-import` mode accepts bbolt-format
...
- **Auth UserDelete** proposes apply tag 16 (username). A missing name is
  fail-closed before propose; apply is idempotent and saves the `auth` bucket.
- **Auth RoleAdd** proposes apply tag 17 (role name). A duplicate name is
  fail-closed before propose; apply is idempotent and saves the `auth` bucket.
- **Auth RoleDelete** proposes apply tag 18 (role name). A missing name is
  fail-closed before propose; apply is idempotent and saves the `auth` bucket.
...
- **Learners** are non-voting: they are excluded from quorum and vote counts.
  ...
  meet the old quorum (etcd `--strict-reconfig-check`, omitted default on;
  `--strict-reconfig-check=false` allows a quorum-losing remove). Learners
  and unknown-without-cluster dispatch are unchanged.
...
- **WAL truncation** — after `snapshot-count` applies (config `0` means 100000),
...
Responses are `:status 200` + gRPC DATA + `grpc-status` trailers. Watch and
...
fail-closes. Status leftover-safe-parses field 5 so leftover
length-delimited bytes cannot steal a printed raftIndex; a truncated
raftIndex fail-closes. Status leftover-safe-parses field 6 so leftover
...
`make_simple_response()` helper used by `Defragment` (after a compact-copy when
a backend is attached) and `MoveLeader` returns a proper
...
`MemberAdd` leftover-safe-parses field 1/2 so leftover length-delimited bytes cannot
steal a peerURL or `isLearner`; a truncated peerURL / isLearner fail-closes
(cannot look like a successful add or a voter). Dummy `0x00` / omitted is empty /
voter.
...
dummy `0x00` is serializable; a truncated varint fail-closes. Linearizable MemberList
requires this node to be leader (same fail-closed empty body as a follower Range).
...
`del --prefix/--prev-kv` (DeleteRange leftover-safe-parses so leftover length-delimited bytes cannot steal `range_end` and turn a point delete into a range delete; truncated `range_end` fail-closes; leftover cannot steal a printed prev_kv key (tag `0x0a` header is not prev key; truncated prev_kv key fail-closes) or a printed delete count or a printed prev_kv value (tag `0x0a` header is not prev value; truncated prev_kv value fail-closes) or a printed prev_kv lease (tag `0x0a` header is not prev lease; truncated prev_kv lease fail-closes) or a printed prev_kv version (tag `0x0a` header raft_term is not prev version; truncated prev_kv version fail-closes) or a printed prev_kv create_revision (tag `0x0a` header member_id is not prev create_revision; truncated prev_kv create_revision fail-closes) or a printed prev_kv mod_revision (tag `0x0a` header revision is not prev mod_revision; truncated prev_kv mod_revision fail-closes)), `put --prev-kv` leftover-safe-skips so leftover cannot steal a printed prev key, `watch --prefix/--prev-kv/--start-rev` (`--start-rev` must be an integer `>= 0`; leftover text fail-closes; WatchCreate leftover-safe-parses so leftover cannot steal `start_rev` / `range_end`); Watch leftover-safe-parses so leftover cannot steal a printed Event key or type or Event lease (tag `0x0a` header raft_term is not Event lease; truncated Event-nested lease fail-closes) or Event version (tag `0x0a` header raft_term is not Event version; truncated Event-nested version fail-closes) or Event create_revision (tag `0x0a` header member_id is not Event create_revision; truncated Event-nested create_revision fail-closes) or Event mod_revision (tag `0x0a` header revision is not Event mod_revision; truncated Event-nested mod_revision fail-closes) or Event value (tag `0x0a` header is not Event value; truncated Event-nested value fail-closes) or Event key (tag `0x0a` header is not Event key; truncated Event-nested key fail-closes) or Event type (tag `0x0a` header cluster_id is not Event type; truncated Event type fail-closes) or Event prev create_revision (tag `0x0a` header member_id is not prev create_revision; truncated Event-nested prev create_revision fail-closes) or Event prev mod_revision (tag `0x0a` header revision is not prev mod_revision; truncated Event-nested prev mod_revision fail-closes) or Event prev version (tag `0x0a` header raft_term is not prev version; truncated Event-nested prev version fail-closes) or Event prev key (tag `0x0a` header is not prev key; truncated Event-nested prev key fail-closes) or Event prev value (tag `0x0a` header is not prev value; truncated Event-nested prev value fail-closes) or `fragment=true` or a printed compact `cancel_reason`; leftover-safe-splits oversized WatchResponses by `--max-request-bytes` when WatchCreate `fragment` is set; compacted WatchCreate / active-watch cancel encodes etcd `ErrCompacted` on field 6; RangeResponse leftover-safe-parses so leftover length-delimited bytes cannot steal a printed `get` / `txn` key or a printed lease (tag `0x0a` header is not lease; truncated lease fail-closes) or a printed version (tag `0x0a` header raft_term is not version; truncated version fail-closes) or a printed create_revision (tag `0x0a` header member_id is not create_revision; truncated create_revision fail-closes) or a printed mod_revision (tag `0x0a` header revision is not mod_revision; truncated mod_revision fail-closes) or a printed value (tag `0x0a` header is not value; truncated value fail-closes) or a printed key (tag `0x0a` header is not key; truncated key fail-closes) or a printed `get --count-only` count / `more=true`; AlarmResponse leftover-safe-parses so leftover cannot steal a printed CORRUPT type; Status leftover-safe-parses so leftover cannot steal a printed alarm error; `txn cas` (compare-and-swap; leftover-safe so leftover cannot steal Compare result and flip a CAS),
`auth login` (token-based authentication; Authenticate leftover-safe-parses so leftover length-delimited bytes cannot steal a name, password, or printed token; truncated password fail-closes), `user add` / `user passwd` leftover-safe-parses so leftover cannot steal a password or `no_password`, `user delete` / `user get` / `role add` / `role delete` / `role get` leftover-safe-parses so leftover cannot steal the name and delete or look up the wrong principal; RoleGet leftover-safe-parses so leftover cannot steal a printed key (tag `0x0a` header is not perm key; truncated perm key fail-closes), permType (tag `0x0a` header cluster_id is not permType; truncated permType fail-closes), or range_end; UserGet / UserList / RoleList leftover-safe-parse first so leftover cannot steal a printed name; Permission leftover-safe-encodes etcd field 2 `key` / field 3 `range_end`, `user grant-role` / `user revoke-role` leftover-safe-parses so leftover cannot steal the user or role, `role grant-permission` / `role revoke-permission` leftover-safe-parses so leftover cannot steal the role, key, range_end, or permType, `get --count-only/--limit N/--sort-by/--sort-order/--print-value-only`,
...
`snapshot restore FILE --data-dir DIR [--wal-dir DIR] [--bump-revision] [--mark-compacted] [--force] [-w json]` (leftover-safe restore; `--data-dir --wal-dir` cannot eat a flag as the path),
`member list --linearizable[=bool]` (etcd default true; leftover-safe; encodes MemberListRequest field 1; a follower fail-closes)
...
`auth status -w json` (JSON output for auth status query); Authenticate leftover-safe-parses so leftover length-delimited bytes cannot steal a used or printed token; AuthStatus leftover-safe-parses so leftover length-delimited bytes cannot steal `enabled` or a printed authRevision (tag `0x0a` header revision is not authRevision; truncated authRevision fail-closes); UserList/RoleList leftover-safe-parses so leftover cannot steal a printed name,
...
`endpoint status -w json|table` (JSON or table output for endpoint status, now parses actual revision from ResponseHeader); Status leftover-safe-parses so leftover length-delimited bytes cannot steal a printed version, dbSize, dbSizeInUse, leader, raftIndex, raftTerm, raftAppliedIndex, or isLearner,
...
`lease revoke -w json` / `lease keepalive -w json` (JSON output for lease revoke and keepalive with ID/TTL fields; lease IDs must be `> 0`; a typo fail-closes),
...
`lock --ttl N` / `elect --ttl N` (customizable lease TTL for distributed lock and leader election, default 60s; `--ttl` must be `> 0`; leftover text fail-closes),
...
All `-w json` commands now leftover-safe-parse ResponseHeader (compact, lease revoke/timetolive/list/grant/keepalive, txn put/del, alarm list, auth enable/disable/status, user/role CRUD, member remove/update/promote, downgrade, defrag, move-leader — leftover length-delimited bytes cannot steal a printed revision).
`hash -w json` / `hashkv -w json` enhanced (now parses ResponseHeader instead of empty `{}`),
`hashkv --rev` / `endpoint hashkv --rev` leftover-safe-sends HashKVRequest.revision (`10foo` fail-closes; omitted / `0` = current; a swallowed `--rev` would hash the wrong tree); the server leftover-safe-parses field 1 so a truncated varint cannot hash the live tree (dummy `0x00` / omitted = current),
`compact` leftover-safe-parses CompactRequest.revision so leftover length-delimited bytes cannot compact a different rev (truncated field 1 fail-closes; `physical` is read and ignored — already sync, not defrag); Hash / HashKV leftover-safe-parses so leftover length-delimited bytes cannot steal a printed hash; `compact` / `hash` / `status` / `defrag` leftover-safe-reject unknown flags (`hash --rev` cannot hash the live tree; `status --cluster` cannot report one node); `defrag --cluster` leftover-safe-sends linearizable MemberList and walks every client URL (a swallowed `--cluster` would defrag only the connected member; a follower cannot walk a stale list); `defrag --data-dir` leftover-safe-opens the local LMDB and compact-copies (`--data-dir --cluster` cannot eat a flag as the path; `--cluster` + `--data-dir` fail-close),
`put` / `get` / `del` / `lease` leftover-safe-reject unknown `--` flags (`put --foo k v` cannot write key `--foo`; `get k --foo` cannot range to `--foo`; `--` starts positionals),
`member` / `watch` / `lock` / `elect` / `snapshot` / `user` / `role` leftover-safe-reject unknown `--` flags (`member remove --force` cannot swallow `--force` and still remove; `watch --foo key` cannot watch `--foo`; `lock --foo name` cannot take `--foo` as LOCKNAME; `--` starts positionals),
`member list --linearizable` leftover-safe-honors etcd's default true and encodes field 1 (`--linearizable --foo` cannot list members; `--linearizable false` is serializable; a follower linearizable list fail-closes),
`txn` / `auth` / `downgrade` / `alarm` leftover-safe-reject unknown `--` flags (`txn put --foo k v` cannot write key `--foo`; `txn put -w json k v` cannot treat `-w` as the key; `auth login --foo` cannot authenticate as `--foo`),
`check perf` / `check datascale` leftover-safe-reject unknown `--` flags (`check perf --foo` cannot start a write load),
`version` leftover-safe-reject unknown `--` flags (`version --foo` cannot print the client version),
`completion` leftover-safe-reject unknown `--` flags (`completion bash --foo` cannot dump a script),
`--flag VALUE` leftover-safe-rejects a next-argv leftover `--` (`--load --prefix` / `--name --data-dir` cannot eat a flag as the value; `--flag=--foo` is still `--foo`),
cetcd leftover-safe-takes remaining server VALUE flags (`--host-whitelist --name` / `--listen-client-urls --name` cannot eat a flag as the list; empty `--host-whitelist=` stays allow-all),
`cetcd-migrate` leftover-safe-parses `--data-dir` / `--output-dir` (`--flag=VALUE` or next argv; `--data-dir --output-dir` cannot eat a flag as the path; empty `--flag=` fail-closes) and leftover-safe-parses etcd `%016x-%016x.wal` so `123foo-456.wal` cannot be scanned and win latest,
IPv6 zone UniqueURLs leftover-safe-split `addr%zone` (`[fe80::1%1]:2379`; empty / `1foo` fail-close) and resolve via getaddrinfo so the scope_id is honored (not stripped),
...
`alarm activate/disarm -w json` (JSON output for alarm activate/disarm with real ResponseHeader); Alarm leftover-safe-parses field 1/2/3 so leftover length-delimited bytes cannot steal ACTIVATE (truncated action fail-closes); AlarmResponse leftover-safe-parses so leftover cannot steal a printed memberID or CORRUPT type; Downgrade leftover-safe-parses field 1/2 so leftover length-delimited bytes cannot steal ENABLE or a VALIDATE version (truncated action / version fail-closes; dummy `0x00` is not a skip length); Downgrade leftover-safe-parses the response so leftover cannot steal a printed cluster version,
...
`etcd-compatible server flags` (cetcd now accepts `--listen-client-urls`, `--listen-peer-urls` as UniqueURLs comma lists (same scheme; unique host:port; port `1..65535`; IPv6 is `http://[::1]:2379` and binds via `uv_ip6_addr`; a hostname (`localhost`) resolves via `getaddrinfo` (IPv4 preferred) so etcd's default listen cannot fail numeric-only bind; IPv6 emit is `[::1]:2379` (advertise / MemberList / cetcdctl endpoint) so `::1:2379` cannot look like port 1; leftover `2379foo` fail-closes; `unix://` / `unixs://` fail-close as known-unsupported (no unix listener; not host `unix://localhost`); `cetcdctl compact`/`hash`/`status`/`defrag` leftover unknown flags fail-close (`hash --rev` cannot hash the live tree; `defrag --cluster` leftover-safe-sends linearizable MemberList (a follower cannot walk a stale list); `defrag --data-dir` leftover-safe-opens the local LMDB and compact-copies (`--data-dir --cluster` cannot eat a flag as the path)); `put`/`get`/`del`/`lease` leftover `--` flags fail-close (`put --foo k v` cannot write key `--foo`; `get k --foo` cannot range to `--foo`); `member`/`watch`/`lock`/`elect`/`snapshot`/`user`/`role` leftover `--` flags fail-close (`member remove --force` cannot swallow `--force` and still remove; `watch --foo key` cannot watch `--foo`); `member list --linearizable` leftover-safe-honors etcd's default true and encodes field 1 (`--linearizable --foo` cannot list members; a follower fail-closes); `role grant/revoke-permission --from-key` leftover-safe-honors Permission.range_end (`--range-end --from-key` cannot eat a flag as the range); HashKV leftover-safe-parses field 1 so a truncated revision varint cannot hash the live tree; Compact leftover-safe-parses field 1 so leftover length-delimited bytes cannot compact a different rev; MoveLeader leftover-safe-parses field 1 so a truncated target cannot look like a successful transfer; LeaseGrant leftover-safe-parses field 1 so a truncated TTL cannot grant a 60s lease; `txn`/`auth`/`downgrade`/`alarm` leftover `--` flags fail-close (`txn put --foo k v` cannot write key `--foo`; `auth login --foo` cannot authenticate as `--foo`); `check` leftover `--` flags fail-close (`check perf --foo` cannot start a write load); `version` leftover `--` flags fail-close (`version --foo` cannot print the client version); `completion` leftover `--` flags fail-close (`completion bash --foo` cannot dump a script); `--flag VALUE` leftover-safe-rejects a next-argv leftover `--` (`--load --prefix` cannot eat a flag as the value); cetcd leftover-safe-takes remaining server VALUE flags (`--host-whitelist --name` cannot eat a flag as the Host list); `cetcd-migrate` leftover-safe-parses `--data-dir`/`--output-dir` and leftover-safe-parses etcd `%016x-%016x.wal` so `123foo-456.wal` cannot win latest (`--flag=VALUE` or next argv; `--data-dir --output-dir` cannot eat a flag as the path); IPv6 zones leftover-safe-split `addr%zone` (`[fe80::1%1]:2379`; empty / `1foo` fail-close) and resolve via getaddrinfo (scope is not stripped); a comma list binds every URL; mixed/duplicate/leftover fail-closes); `--initial-cluster` and MemberAdd/Update peer URL ports the same (IPv6 brackets leftover-safe); member ids must be `> 0`; `--advertise-client-urls` / `--initial-advertise-peer-urls` are UniqueURLs comma lists on MemberList (repeated strings; empty defaults from every listen URL; mixed scheme allowed; `https://` requires the matching cert file; leftover/duplicate fail-closes); `--name` fills MemberList self name (empty → `default`); remaining space-only value flags (`--name`, `--data-dir`, `--wal-dir`, `--snapshot-count`, `--initial-cluster`, `--cert-file`, `--auto-compaction-*`, `--max-txn-ops`, …) accept etcd `--flag=value` (empty `--flag=` fail-closes; `--initial-cluster=n1=http://host:2380` keeps the member `=`); `--logger` is `zap` or `capnslog` (built-in logger; other types fail at parse); `--log-level` is `trace`/`debug`/`info`/`warn`/`error` (`=` form accepted) (etcd `warning`/`dpanic`/`panic`/`fatal` aliases; other levels fail at parse); `--log-format` is `json` or `text` (etcd `console` = text; other formats fail at parse); `--enable-log-rotation` rotates a single `--log-outputs` file (`--log-rotation-config-json` lumberjack; `compress:true` fail-closes); `--initial-cluster-token` is persisted in `data-dir` and a mismatch fail-closes; `--experimental-initial-corrupt-check` compares HashKV to `{data-dir}/backend.hash` after WAL replay; `--experimental-corrupt-check-time` repeats that HashKV on the tick; `--experimental-compaction-batch-limit` caps auto-compact revs per tick; `--experimental-compaction-sleep-interval` waits between those batches; `--experimental-watch-progress-notify-interval` sets Watch progress_notify; `--experimental-warning-apply-duration` warns on slow apply; `--experimental-warning-unary-request-duration` warns on slow unary RPC; `--experimental-max-learners` caps learner MemberAdd; `--experimental-memory-mlock` locks process memory; `--experimental-bootstrap-defrag-threshold-megabytes` compact-copies data.mdb at start; `--experimental-wait-cluster-ready` delays client listen until a Raft leader exists (omitted default off; a non-bool fail-closes); `--experimental-enable-lease-checkpoint` / `--experimental-enable-lease-checkpoint-persist` honor remaining TTL already raft-applied and LMDB-persisted (`true`/`false`; a non-bool fail-closes; unimplemented or unknown `--experimental-*` fail at parse; `=false` is OK); `--auto-tls`/`--peer-auto-tls` mint ECDSA P-256 into `{data-dir}/fixtures/` when cert flags are empty (`=` form accepted; `=false` is off, not unknown) (reuse if both files exist; one-without-the-other fail-closes); `--self-signed-cert-validity` sets mint lifetime in years (`> 0`; omitted default 1); `--grpc-keepalive-time`/`--grpc-keepalive-interval`/`--grpc-keepalive-timeout` apply TCP keepalive on accepted client sockets, accepted peer sockets, and outbound Raft dials (`interval` is the etcd idle name; Go durations such as `2h` are accepted; timeout without time/interval fail-closes); other `--grpc-keepalive-*` are unknown flags and do not swallow the next argv (`--grpc-keepalive-permit-without-stream` is a bool; a non-boolean value fail-closes); `--log-outputs` is stderr, stdout, a file path, or journal/syslog/systemd/journal (`=` form accepted; etcd `default` fail-closes; mixed comma-lists fail-close); `--wal-dir` places the WAL off `data-dir` (empty path fail-closes); `--discovery-srv` / `--discovery-srv-name` bootstrap peers from DNS SRV (mixed with `--initial-cluster` fail-closes); `--initial-cluster-state existing` requires persisted cluster evidence (empty dir fail-closes); `--force-new-cluster` keeps MVCC and drops peers except self (empty dir fail-closes; not a wipe; `=` form accepted; `=false` is off); `--strict-reconfig-check` rejects a quorum-losing MemberRemove (omitted default on; `--strict-reconfig-check=false` allows it; a non-bool fail-closes); `--port` / `--peer-port` are `1..65535` (a typo fail-closes instead of binding port `0`); `--metrics-port` is `0..65535` (`0` disables; a typo fail-closes); `--listen-metrics-urls` is a UniqueURLs comma list of `http(s)://host:port` (port `1..65535`; a comma list binds every URL; mixed scheme allowed; `https://` terminates TLS with `--cert-file` / `--auto-tls` and no ALPN; missing cert / duplicate / leftover fail-closes; cannot mix with `--metrics-port`; binds `/metrics` and `/health`); `--enable-pprof` is true/false (bare flag on; omitted default off; `/debug/pprof/*` 404 unless on; a non-bool fail-closes); `--host-whitelist` is a comma-separated Host list on the metrics port (omitted / `*` / empty = allow all; a restricted list 403s a missing or unknown Host); `--metrics` is `basic` or `extensive` (omitted default `basic`; extensive observes unary `grpc_server_handling_seconds`; other values fail-close); `--socket-reuse-port` sets `SO_REUSEPORT` on listeners (omitted default off; Windows fail-closes; a non-bool fail-closes); `--socket-reuse-address` `true`/bare is accepted (libuv already sets `SO_REUSEADDR`); `false` fail-closes; `--enable-grpc-gateway=false` / `--enable-v2=false` / `--unsafe-no-fsync=false` are accepted (already off); bare/`true` fail-closes; `--listen-client-http-urls` fail-closes (no gRPC-gateway); `--max-snapshots` / `--max-wals` / `--client-cert-file` / `--client-key-file` / `--backend-batch-*` / `--backend-bbolt-freelist-type` fail-close (single WAL; no outbound client TLS; LMDB); `--v2-deprecation` is `gone`/`write-only` (and drop/skip-check aliases); `not-yet` fail-closes; `--proxy=off` / `--discovery-fallback=exit` are accepted; `on`/`readonly`/`proxy` / `--discovery` (v2 URL) / `--cors` fail-close; `--raft-read-timeout` / `--raft-write-timeout` recycle hung peer sockets (Go duration; omitted / `<5s` floor to 5s; leftover fail-closes); `--experimental-snapshot-catchup-entries` keeps N raft entries after compact (omitted default 5000; `0` compact to applied; leftover fail-closes); `--experimental-compact-hash-check-enabled` / `--experimental-compact-hash-check-time` compare follower compact HashKV (omitted default off / 1m; mismatch raises CORRUPT; leftover fail-closes); `--node-id` must be `> 0` (a typo fail-closes instead of Raft id `0`); `--election-tick` must be `> 0` (a typo or `0` fail-closes instead of becoming `10`); `--heartbeat-tick` must be `> 0` (a typo or `0` fail-closes instead of becoming `1`); `--heartbeat-interval` / `--election-timeout` are integer milliseconds (`1..50000`; omitted 100/1000; election must be `>=` interval; leftover text fail-closes; cannot mix with `--*-tick`; they set the timer period and `election_tick`); `--initial-election-tick-advance` is true/false (bare flag on; omitted default on; ticks `election_tick-1` at create; `=false` waits the full timeout; a non-bool fail-closes); `--pre-vote` is true/false (bare flag on; omitted default on; a non-bool fail-closes); `--snapshot-count` must be `> 0` (a typo or `0` fail-closes; omitted is etcd 3.5's 100000; `=` form accepted); `--auto-compaction-mode` is `periodic` or `revision` and `--auto-compaction-retention` is `0` (off), a periodic duration / bare hours, or a revision count (invalid values fail-close; the leader compact-proposes on tick); `--max-request-bytes` caps the client read buffer (default 1.5 MiB) and closes oversized frames (`> 0` at parse; a typo or `0` fail-closes); `--max-concurrent-streams` advertises HTTP/2 `SETTINGS_MAX_CONCURRENT_STREAMS` (`> 0` at parse; clamped to `CETCD_H2_MAX_STREAMS`; omitted leaves nghttp2's default; `0` or leftover text fail-closes; each stream keeps its own path/token/body); `--max-txn-ops` rejects Txn lists longer than N (default 128, hard max 128; `1..128` at parse; a typo or `0` fail-closes); `--quota-backend-bytes` raises NOSPACE and rejects Puts when LMDB size is at or above the cap (Deletes still apply; `0` / omitted = etcd 2GiB; `=` form accepted; a typo fail-closes); `--auth-token jwt,sign-method=HS256|RS256|ES256,priv-key=PATH[,ttl=5m]` issues JWTs (other methods fail closed); `--auth-token-ttl` sets simple-token lifetime in seconds (`> 0`; omitted default 300; leftover text fail-closes; JWT `ttl=` still wins); `--bcrypt-cost` hashes new passwords with bcrypt (`0` or `4..31`; a typo fail-closes); `--cert-file`/`--key-file`/`--trusted-ca-file`/`--client-cert-auth` and the matching `--peer-*` flags terminate TLS on accept and on outbound `peer_tx_` (`--trusted-ca-file` / `--peer-trusted-ca-file` require a client/peer cert; `=false` does not opt out); `--cipher-suites` restricts those TLS contexts with IANA or OpenSSL names (TLS 1.3 IANA names use `SSL_CTX_set_ciphersuites`; a TLS 1.3-only list disables TLS 1.2 and a TLS 1.2-only list disables TLS 1.3) and fail-closes on an unknown list or when TLS is not configured; `--tls-min-version` / `--tls-max-version` are `TLS1.2` or `TLS1.3` (omitted min TLS1.2; omitted max open; `TLS1.1` or min > max fail-closes); `--peer-cert-allowed-cn` / `--peer-cert-allowed-hostname` / `--client-cert-allowed-hostname` close a handshake whose peer cert CN/SAN is not listed (restricted lists require cert+CA; leftover fail-closes); `--peer-client-cert-file` / `--peer-client-key-file` present a distinct outbound peer cert (omitted uses `--peer-cert-file`; cert without key or override without listen TLS fail-closes); `--client-crl-file` / `--peer-crl-file` reject a handshake whose peer cert serial is on the CRL (requires that side's cert; missing/garbage CRL fail-closes); `--version` prints `etcd Version:` and exits; `--config-file` is an etcd YAML map of flag names applied through the same parser (other CLI flags and `ETCD_*` ignored; nested/missing fail-closes); `ETCD_*` maps to `--flag` when no config file is set (`ETCD_LISTEN_CLIENT_URLS`; empty ignored; `ETCD_VERSION` skipped; CLI+env conflict or a typo fail-closes); `https://` on `--listen-client-urls` / `--listen-peer-urls` requires the matching cert files; `https://` in `--initial-cluster` requires `--peer-cert-file`; unknown flags fail at parse rather than being ignored; `--wal-dir` is accepted),
...
`snapshot file format with revision` (snapshot save writes a 16-byte CTS2 header: magic "CTS2" + 8-byte revision LE + 4-byte CRC32C LE of the kv blob; `snapshot status` shows the stored CRC for CTS2 or computes CRC32C of the body for CTS1/raw; `snapshot restore` verifies CTS2 unless `--skip-hash-check`, then writes CTS2 (known/bumped revision) or raw KV to `snapshot.kv`; `--bump-revision` advances MVCC so a new cluster cannot reuse the snapshot revision; `--mark-compacted` without bump fail-closes; `--wal-dir` is persisted; leftover `--data-dir --wal-dir` fail-closes; legacy CTS1 and headerless files still restore),
...
`move-leader -w fields` (fields output showing ResponseHeader; TARGET_ID is hex `> 0`; leftover text fail-closes); MoveLeader leftover-safe-parses field 1 so a truncated varint cannot look like a successful transfer (dummy `0x00` / `0` fail-closes; leftover length-delimited bytes cannot steal the target),
`snapshot status -w fields` (fields output showing hash, revision, total_keys, size, filename),
`downgrade enable/cancel/validate -w fields` (fields output showing ResponseHeader and leftover-safe version; leftover cannot steal a printed cluster version),
...
`lease grant --lease-id ID` (custom lease ID in hex format, matching etcdctl behavior; a non-hex id fail-closes; grant TTL must be `> 0`; a typo fail-closes); LeaseGrant leftover-safe-parses field 1 so a truncated TTL cannot grant a 60s lease (dummy `0x00` / `TTL<=0` fail-closes); LeaseGrant/TimeToLive leftover-safe-parses the response so leftover length-delimited bytes cannot steal a printed or lock-used ID or key (tag `0x08` is not the grant ID); `cetcdctl lease grant` / `timetolive` leftover-safe-parse json/fields so leftover cannot steal a printed ID, TTL key, or grant error; TimeToLive leftover-safe-parses field 5 so leftover cannot steal a printed TTL key; TimeToLive leftover-safe-parses field 4 so leftover cannot steal a printed grantedTTL (tag `0x0a` header raft_term is not grantedTTL); truncated grantedTTL fail-closes; TimeToLive leftover-safe-parses field 3 so leftover cannot steal a printed remaining TTL (tag `0x0a` header revision is not remaining TTL); truncated remaining TTL fail-closes; TimeToLive leftover-safe-parses field 2 so leftover cannot steal a printed lease ID (tag `0x0a` header member_id is not the lease ID); truncated ID fail-closes; KeepAlive leftover-safe-parses so leftover cannot steal a printed or lock-used TTL; LeaseLeases leftover-safe-parses so leftover cannot steal a printed ID; LeaseRevoke/KeepAlive/TimeToLive leftover-safe-parses field 1 so a truncated id cannot look like a successful keepalive or steal a revoke (dummy `0x00` / `0` fail-closes KeepAlive),
...
`role grant-permission --prefix` / `--from-key` / `--range-end KEY` / ENDKEY leftover-safe-honors Permission.range_end so `--range-end --from-key` cannot eat a flag as the range; a swallowed range_end cannot grant a prefix instead of [key, range_end) (FromKey is a single 0),
`role revoke-permission ROLE [TYPE] KEY [ENDKEY]` leftover-safe-honors `--from-key` / `--range-end` / ENDKEY so `--range-end --from-key` cannot eat a flag as the range (sends key as field 2 and range_end as field 3),
...
`completion bash|zsh|fish` (generate shell completion scripts for bash, zsh, and fish shells, covering all cetcdctl commands, subcommands, global options, and command-specific flags; leftover `completion bash --foo` fail-closes),
...
`member add --peer-urls url1,url2` (comma-separated multiple peer URLs for cluster member addition; `member update` also supports comma-separated URLs; peer URL ports are leftover-safe `1..65535`; `member remove`/`update`/`promote` IDs are hex `> 0`; leftover text fail-closes); MemberRemove/Promote leftover-safe-parses field 1 so a truncated varint cannot look like a successful remove or promote (dummy `0x00` / `0` fail-closes); MemberUpdate leftover-safe-parses field 1/2 so a truncated id or leftover peerURL length cannot look like a successful update; MemberAdd leftover-safe-parses field 1/2 so leftover length-delimited bytes cannot steal a peerURL or `isLearner` (truncated peerURL / isLearner fail-closes); MemberList leftover-safe-parses so leftover length-delimited bytes cannot steal a printed peerURL, id, `isLearner`, a used `--cluster` client URL, or a printed member name (truncated peerURL / isLearner / clientURL / name / id fail-closes; tag `0x0a` header is not isLearner; tag `0x0a` header cluster_id is not member id; tag `0x0a` header is not peerURL); Member leftover-safe-encodes etcd field 2 `name` / field 3 `peerURLs`,
...
empty range_end.
DeleteRange leftover-safe-parses so leftover length-delimited bytes cannot steal
a printed prev_kv key; a truncated prev_kv / key fail-closes (cannot print leftover
text). Dummy `0x00` / omitted is 0 prev_kvs.
...
matching etcd — instead of treating the missing value as empty bytes. An empty
compare range with a non-`VALUE` target compares against a zero KeyValue (not a
vacuous success). The
...
(`revision <= compacted_rev`), or with omitted/`0` revision (etcd
`rev <= compactMainRev` when the boundary is still 0), likewise fails —
matching etcd ErrCompacted — instead of returning a successful no-op. `LeaseRevoke` of a
...
a name, password, or printed token; a truncated password fail-closes (cannot look like a name-only
authenticate). Dummy `0x00` / omitted is empty. UserAdd leftover-safe-parses
so leftover cannot steal a password or `no_password`; UserChangePassword
leftover-safe-parses the same name/password fields. UserDelete / RoleAdd /
RoleDelete / UserGet / RoleGet leftover-safe-parses so leftover
length-delimited bytes cannot steal the name and delete or look up the
wrong principal; a truncated name fail-closes (cannot look like a
successful delete). UserGrantRole / UserRevokeRole leftover-safe-parses
the same user/role strings so leftover cannot grant or revoke the wrong
...
messages — create, cancel, and fallback paths. Watch leftover-safe-parses so leftover
length-delimited bytes cannot steal a printed Event key or type or Event lease or Event version; a truncated
event / kv / key / Event-nested lease / Event-nested version / Event-nested create_revision / Event-nested mod_revision / Event-nested prev create_revision / Event-nested prev mod_revision / Event-nested prev version / Event-nested prev key / Event-nested prev value fail-closes (cannot print leftover text). The Event `KeyValue` encoding uses correct
...
Watch leftover-safe-parses WatchResponse.fragment (field 7) so leftover
length-delimited bytes cannot steal `fragment=true`; a truncated fragment
fail-closes. When `fragment` is set, streaming notify leftover-safe-splits
oversized WatchResponses by `--max-request-bytes` and sets `fragment=true`
on non-last frames (one event over budget is still sent). `cetcdctl watch
--fragment` leftover-safe-encodes field 8 (not a no-op).
Watch leftover-safe-parses `cancel_reason` (field 6) so leftover
length-delimited bytes cannot steal a printed compact cancel; a truncated
reason fail-closes. Compacted WatchCreate and compacted active-watch
cancel encode etcd `ErrCompacted` (`etcdserver: mvcc: required revision
has been compacted`). `cetcdctl watch` leftover-safe-parses field 6.
...
WatchCreate returns `created=true`, `canceled=true`, `compact_revision` (field 5, tag
`0x28`), and `cancel_reason` (field 6, etcd `ErrCompacted`) without subscribing —
matching etcd's ErrCompacted watch behavior.
...
Prometheus text exposition format on demand. `GET /health` returns etcd JSON
(`health` as a string). NOSPACE, CORRUPT, or no Raft leader is HTTP 503;
`?serializable=true` skips the leader check and `exclude=NOSPACE|CORRUPT`
skips that alarm. `--host-whitelist` (omitted / `*` = allow all) 403s an
unknown `Host` on this port. `--metrics basic` (omitted default) skips
unary histograms; `--metrics extensive` observes
`grpc_server_handling_seconds` on unary dispatch (DefBuckets). Other
values fail at parse. `--socket-reuse-port` (omitted default off) sets
`SO_REUSEPORT` on client, peer, and metrics binds. Windows fail-closes.
`--raft-read-timeout` / `--raft-write-timeout` (omitted default 5s;
values `<5s` floor to 5s) close a hung inbound/outbound peer read or
an in-flight outbound write on the Raft tick. Leftover text fail-closes.
`--experimental-snapshot-catchup-entries` (omitted default 5000; `0`
compact to applied) keeps that many raft entries after WAL compact so
a slightly-behind follower can `App` instead of `MsgSnap`. Leftover
text fail-closes. `--experimental-compact-hash-check-enabled` (omitted default off)
with `--experimental-compact-hash-check-time` (omitted 1m) HashKVs the
compacted revision on each member and sends it to peers; the leader
raises CORRUPT on a same-rev mismatch.
`--experimental-enable-lease-checkpoint` /
`--experimental-enable-lease-checkpoint-persist` accept `true`/`false`
(bare is true). Remaining TTL is already raft-applied and written to
the LMDB `lease` bucket, so `true` is that path, not a no-op.
Unimplemented or unknown `--experimental-*` fail at parse
(`enable-distributed-tracing*`,
`stop-grpc-service-on-defrag`, `peer-skip-client-san-verification`,
`txn-mode-write-with-shared-buffer`, `enable-v2v3`,
`downgrade-check-time`). Bool forms accept `false`/`0`. The old
catch-all no longer swallows the next argv.
`--listen-metrics-urls` `https://` terminates TLS on that listener
with `--cert-file` / `--auto-tls` (same CA/CRL/cipher/version /
client-cert-auth; no ALPN). Mixed http/https is allowed. Missing
cert fail-closes. An `http://` client listen is not wrapped just
because metrics needs certs.
...
`cetcdctl subcommand --flag=value` (put/get/del/watch/compact/lease/lock/elect/status/txn/endpoint/check/member/auth/user/role/snapshot/alarm/hash/defrag/move-leader accept `--lease=1`, `--rev=5`, `--write-out=json`, `--ttl=60`, `--start-rev=5`, `--data-dir=DIR`, `--load=N`; leftover `put --lease 10foo` / `watch cancel 10foo` / `--load=10foo` fail-closes; empty `--flag=` fail-closes; `--prefix=false` does not eat the key; `compact`/`hash`/`status`/`defrag` unknown leftover flags fail-close; `defrag --cluster` leftover-safe-sends linearizable MemberList; `defrag --data-dir` leftover-safe-opens the local LMDB; `put`/`get`/`del`/`lease` unknown `--` flags fail-close so they cannot become a key or range_end; `member`/`watch`/`lock`/`elect`/`snapshot`/`user`/`role` unknown `--` flags fail-close so `--force` cannot remove a member and `--foo` cannot become a watch key; `txn`/`auth`/`downgrade`/`alarm` unknown `--` flags fail-close so `txn put --foo` cannot write key `--foo` and `auth login --foo` cannot authenticate as `--foo`; `check` unknown `--` flags fail-close so `check perf --foo` cannot start a write load; `version` leftover `--` flags fail-close so `version --foo` cannot print the client version; `completion` leftover `--` flags fail-close so `completion bash --foo` cannot dump a script; `--flag VALUE` leftover-safe-rejects a next-argv leftover `--` so `--load --prefix` cannot eat a flag as the value; cetcd leftover-safe-takes remaining server VALUE flags so `--host-whitelist --name` cannot eat a flag as the Host list; `cetcd-migrate` leftover-safe-parses `--data-dir`/`--output-dir` and leftover-safe-parses etcd `%016x-%016x.wal` so `123foo-456.wal` cannot win latest so `--data-dir --output-dir` cannot eat a flag as the path),
`get --count-only -w fields` (fields output now includes ResponseHeader fields and `"count" : N` line, matching etcdctl fields format),
`snapshot restore` leftover-safe `--wal-dir` / `--bump-revision` / `--mark-compacted` (`--data-dir --wal-dir` cannot eat a flag as the path; bump writes CTS2 at old+1 and advances MVCC; `--mark-compacted` without bump fail-closes); `member list --linearizable` leftover-safe-honors etcd's default true and encodes field 1 (`--linearizable --foo` cannot list members; a follower fail-closes); `snapshot restore` etcd-compatible flags (`--initial-cluster-token` writes `{data-dir}/cluster_token` and a mismatch without `--force` fail-closes; `--initial-cluster-state` is `new` or `existing` and is persisted; `--initial-cluster` / `--name` / `--initial-advertise-peer-urls` are validated and written to the data dir; unknown state, empty values, a bad spec, or a mismatch without `--force` fail-close; `--skip-hash-check` overrides a CTS2 CRC mismatch; a truncated CTS2 header still fail-closes),
