...
multiplexing). `--max-concurrent-streams N` (`N > 0`) advertises
`SETTINGS_MAX_CONCURRENT_STREAMS` on each new session; omitted leaves
nghttp2's default. After TCP accept, the first bytes are classified with
