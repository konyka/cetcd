...
`compact` / `hash` / `status` / `defrag` leftover-safe-reject unknown flags (`hash --rev` cannot hash the live tree; `status --cluster` cannot report one node); `defrag --cluster` walks MemberList (a swallowed `--cluster` would defrag only the connected member),
