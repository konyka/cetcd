...
| Raft | State machine + peer TCP framing; Put/DeleteRange propose, apply after WAL sync. Server `pre_vote` runs a PreVote round (no term bump) before a real election (`--pre-vote=false` skips it; omitted default on); `TIMEOUT_NOW` skips it. |
