...
(`123foo-456.snap` / decimal `atoll` cannot win latest) and leftover-safe-parses
...
`cetcd-migrate` leftover-safe-parses `--data-dir` / `--output-dir` (`--flag=VALUE` or next argv; `--data-dir --output-dir` cannot eat a flag as the path; empty `--flag=` fail-closes),
