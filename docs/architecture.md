...
`compact` / `hash` / `status` / `defrag` leftover-safe-reject unknown flags (`hash --rev` cannot hash the live tree; `status --cluster` cannot report one node); `defrag --cluster` walks MemberList (a swallowed `--cluster` would defrag only the connected member); `defrag --data-dir` leftover-safe-opens the local LMDB and compact-copies (`--data-dir --cluster` cannot eat a flag as the path; `--cluster` + `--data-dir` fail-close),
