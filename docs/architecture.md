...
| TLS accept | Memory-BIO termination on client/peer listen. Cert without key, missing files, or `--client-cert-auth` without CA fail closed. Outbound `peer_tx_` is still plaintext. |
