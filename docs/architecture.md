...
| Cluster join | `--initial-cluster-state existing` restarts from evidence or starts as a follower with `--initial-cluster` peers. `snapshot.kv` is imported into empty MVCC. After WAL compaction the leader sends `MsgSnap` (KV blob) to a lagging joiner; while the log is still live it sends `App` from `next_idx`. |
...
`snapshot restore` etcd-compatible flags (`--initial-cluster-token` writes `{data-dir}/cluster_token` and a mismatch without `--force` fail-closes; `--initial-cluster-state` is `new` or `existing`; unknown values fail-close; `--skip-hash-check`, `--initial-cluster`, `--initial-advertise-peer-urls`, `--name` stay no-op),
