...
- **Range leftover-safe KV value** — leftover-safe-parses
  RangeResponse KV value so leftover length-delimited bytes cannot
  steal a printed value. A truncated value fail-closes (cannot print
  leftover text). Dummy `0x00` is not a skip length. Tag `0x0a` header
  is not value. `cetcdctl get` / `put --prev-kv` / `del --prev-kv` /
  `watch` leftover-safe-parse field 5. Accepting leftover payload as
  the printed value is rejected.
- **Range leftover-safe KV key** — leftover-safe-parses
  RangeResponse KV key so leftover length-delimited bytes cannot
  steal a printed key. A truncated key fail-closes (cannot print
  leftover text). Dummy `0x00` is not a skip length. Tag `0x0a` header
  is not key. `cetcdctl get` / `put --prev-kv` / `del --prev-kv` /
  `watch` leftover-safe-parse field 1. Accepting leftover payload as
  the printed key is rejected.
...
- **Status leftover-safe dbSize** — leftover-safe-parses
  StatusResponse.dbSize so leftover length-delimited bytes cannot
  steal a printed allocated size. A truncated dbSize fail-closes
  (cannot look like an empty allocated size). Dummy `0x00` is not a
...
- **TimeToLive leftover-safe remaining TTL** — leftover-safe-parses
  LeaseTimeToLiveResponse.TTL so leftover length-delimited bytes
  cannot steal a printed remaining TTL. A truncated remaining TTL
  fail-closes (cannot look like remaining 0). Dummy `0x00` is not a
...
- **Watch leftover-safe Event KV create_revision** — leftover-safe-parses
  WatchResponse Event KV create_revision so leftover length-delimited
  bytes cannot steal a printed Event create_revision. A truncated
  Event-nested create_revision fail-closes (cannot look like
  create_revision 0). Dummy `0x00` is not a skip length. Tag `0x0a`
  header member_id is not Event create_revision. `cetcdctl watch`
  leftover-safe-parses field 2 inside Event. Accepting leftover payload
  as the printed Event create_revision is rejected.
- **Watch leftover-safe Event KV mod_revision** — leftover-safe-parses
  WatchResponse Event KV mod_revision so leftover length-delimited
  bytes cannot steal a printed Event mod_revision. A truncated
  Event-nested mod_revision fail-closes (cannot look like
  mod_revision 0). Dummy `0x00` is not a skip length. Tag `0x0a`
...
- **Watch leftover-safe Event KV value** — leftover-safe-parses
  WatchResponse Event KV value so leftover length-delimited bytes
  cannot steal a printed Event value. A truncated Event-nested value
  fail-closes (cannot print leftover text). Dummy `0x00` is not a skip
  length. Tag `0x0a` header is not Event value. `cetcdctl watch`
  leftover-safe-parses field 5 inside Event. Accepting leftover payload
  as the printed Event value is rejected.
- **Watch leftover-safe Event KV key** — leftover-safe-parses
  WatchResponse Event KV key so leftover length-delimited bytes
  cannot steal a printed Event key. A truncated Event-nested key
  fail-closes (cannot print leftover text). Dummy `0x00` is not a skip
...
- **Watch leftover-safe Event prev_kv create_revision** —
  ...
  create_revision is rejected.
- **Watch leftover-safe Event prev_kv mod_revision** —
  leftover-safe-parses WatchResponse Event prev_kv mod_revision so
  leftover length-delimited bytes cannot steal a printed prev
  mod_revision. A truncated Event-nested prev mod_revision
  fail-closes (cannot look like mod_revision 0). Dummy `0x00` is
  not a skip length. Tag `0x0a` header revision is not prev
...
- **Watch leftover-safe Event prev_kv key** — leftover-safe-parses
  WatchResponse Event prev_kv key so leftover length-delimited bytes
  cannot steal a printed prev key. A truncated Event-nested prev key
  fail-closes (cannot print leftover text). Dummy `0x00` is not a skip
...
- **DeleteRange leftover-safe prev_kv value** — leftover-safe-parses
  ...
  payload as the printed prev value is rejected.
- **DeleteRange leftover-safe prev_kv lease** — leftover-safe-parses
  DeleteRangeResponse prev_kv lease so leftover length-delimited
  bytes cannot steal a printed prev lease. A truncated prev_kv lease
  fail-closes (cannot look like lease 0). Dummy `0x00` is not a skip
