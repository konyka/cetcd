...
static void parse_range_response(const uint8_t *data, size_t len) {
    ...
    if (cetcd_parse_range_response_kv_mod_rev(data, len, &leftover_mod)
        ...
        return;
    (void)leftover_mod;
    char leftover_value[256];
    leftover_value[0] = '\0';
    /* leftover-safe: leftover cannot steal a printed value */
    if (cetcd_parse_range_response_kv_value(data, len, leftover_value,
...
static int cmd_put(int argc, char **argv) {
    ...
    leftover_value[0] = '\0';
    /* leftover-safe: leftover cannot steal a printed value */
    if (cetcd_parse_range_response_kv_value(resp, (size_t)rlen, leftover_value,
                                           sizeof(leftover_value)) != CETCD_OK) {
        fprintf(stderr, "request failed\n");
        if (stdin_val) free(stdin_val);
        return 1;
...
static int cmd_del(int argc, char **argv) {
    ...
    (void)leftover_key;
    char leftover_prev_value[256];
    leftover_prev_value[0] = '\0';
    /* leftover-safe: leftover cannot steal a printed prev value */
    if (cetcd_parse_delete_range_prev_kv_value(resp, (size_t)rlen,
                                               leftover_prev_value,
                                               sizeof(leftover_prev_value))
        != CETCD_OK) {
        fprintf(stderr, "request failed\n");
        return 1;
    }
    (void)leftover_prev_value;
    int64_t leftover_prev_lease = 0;
    /* leftover-safe: leftover cannot steal a printed prev lease */
    if (cetcd_parse_delete_range_prev_kv_lease(resp, (size_t)rlen,
                                               &leftover_prev_lease)
        != CETCD_OK) {
        fprintf(stderr, "request failed\n");
        return 1;
...
static int print_watch_response(const uint8_t *resp, size_t rlen,
    ...
    if (cetcd_parse_range_response_kv_lease(resp, rlen, &leftover_lease)
        ...
        leftover_lease = 0;
    (void)leftover_lease;
    int64_t leftover_event_lease = 0;
    /* leftover-safe: leftover cannot steal a printed Event lease */
    if (cetcd_parse_watch_event_kv_lease(resp, rlen, &leftover_event_lease)
        != CETCD_OK)
        return 0;
    (void)leftover_event_lease;
    int64_t leftover_event_version = 0;
    /* leftover-safe: leftover cannot steal a printed Event version */
    if (cetcd_parse_watch_event_kv_version(resp, rlen, &leftover_event_version)
        != CETCD_OK)
        return 0;
    (void)leftover_event_version;
    int64_t leftover_event_create = 0;
    /* leftover-safe: leftover cannot steal a printed Event create_revision */
    if (cetcd_parse_watch_event_kv_create_rev(resp, rlen, &leftover_event_create)
        != CETCD_OK)
        return 0;
    (void)leftover_event_create;
    int64_t leftover_event_mod = 0;
    /* leftover-safe: leftover cannot steal a printed Event mod_revision */
    if (cetcd_parse_watch_event_kv_mod_rev(resp, rlen, &leftover_event_mod)
        != CETCD_OK)
        return 0;
    (void)leftover_event_mod;
    char leftover_event_value[256];
    leftover_event_value[0] = '\0';
    /* leftover-safe: leftover cannot steal a printed Event value */
    if (cetcd_parse_watch_event_kv_value(resp, rlen, leftover_event_value,
                                        sizeof(leftover_event_value))
        != CETCD_OK)
        return 0;
    ...
    if (cetcd_parse_watch_event_prev_kv_create_rev(resp, rlen,
                                                  &leftover_event_prev_create)
        != CETCD_OK)
        return 0;
    (void)leftover_event_prev_create;
    int64_t leftover_event_prev_mod = 0;
    /* leftover-safe: leftover cannot steal a printed Event prev mod_revision */
    if (cetcd_parse_watch_event_prev_kv_mod_rev(resp, rlen,
                                               &leftover_event_prev_mod)
        != CETCD_OK)
        return 0;
    (void)leftover_event_prev_mod;
    int64_t leftover_event_prev_version = 0;
    /* leftover-safe: leftover cannot steal a printed Event prev version */
    if (cetcd_parse_watch_event_prev_kv_version(resp, rlen,
                                               &leftover_event_prev_version)
        != CETCD_OK)
        return 0;
    (void)leftover_event_prev_version;
    char leftover_event_prev_key[128];
    leftover_event_prev_key[0] = '\0';
    /* leftover-safe: leftover cannot steal a printed Event prev key */
    if (cetcd_parse_watch_event_prev_kv_key(resp, rlen, leftover_event_prev_key,
                                           sizeof(leftover_event_prev_key))
        != CETCD_OK)
        return 0;
    (void)leftover_event_prev_key;
    char leftover_event_prev_value[128];
    leftover_event_prev_value[0] = '\0';
    /* leftover-safe: leftover cannot steal a printed Event prev value */
    if (cetcd_parse_watch_event_prev_kv_value(resp, rlen,
                                             leftover_event_prev_value,
                                             sizeof(leftover_event_prev_value))
        != CETCD_OK)
        return 0;
    ...
    leftover_value[0] = '\0';
    /* leftover-safe: leftover cannot steal a printed value */
    if (cetcd_parse_range_response_kv_value(resp, rlen, leftover_value,
                                           sizeof(leftover_value)) != CETCD_OK)
        leftover_value[0] = '\0';
    (void)leftover_value;
    char leftover_key[256];
