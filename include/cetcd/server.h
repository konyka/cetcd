...
int cetcd_parse_range_response_kv_version(const uint8_t *req, size_t len,
                                          int64_t *version);
/* leftover-safe RangeResponse KV create_revision (field 3, tag 0x10).
 * omitted / empty / dummy 0x00 = 0. leftover truncated
 * create_revision is INVAL so a truncated get cannot look like
 * create_revision 0. leftover length-delimited fields are skipped by
 * payload so leftover bytes cannot steal a printed create_revision
 * (tag 0x0a header member_id is not create_revision). unknown wire
 * types are INVAL. last create_revision is copied when create_rev is
 * set. */
...
/* leftover-safe RangeResponse KV key (field 1, tag 0x0a). omitted /
 * empty / dummy 0x00 = empty key. leftover truncated key is INVAL so
 * a truncated get cannot print leftover text as the key. leftover
 * length-delimited fields are skipped by payload so leftover bytes
 * cannot steal a printed key (tag 0x0a header is not key). unknown
 * wire types are INVAL. last key is copied when key/key_cap are
 * set. */
 ...
 * length-delimited fields are skipped by payload so leftover bytes
 * cannot steal a printed leader. unknown wire types are INVAL. last
 * leader is copied when leader is set. */
...
int cetcd_parse_status_raft_index(const uint8_t *req, size_t len,
                                  uint64_t *raft_index);
/* leftover-safe StatusResponse.raftTerm (field 6, tag 0x30). omitted /
 * empty / dummy 0x00 = 0. leftover truncated raftTerm is INVAL so a
 * truncated status cannot look like raftTerm 0. leftover
 * length-delimited fields are skipped by payload so leftover bytes
 * cannot steal a printed raftTerm. unknown wire types are INVAL.
 * last raftTerm is copied when raft_term is set. */
...
int cetcd_parse_status_raft_term(const uint8_t *req, size_t len,
                                 uint64_t *raft_term);
/* leftover-safe StatusResponse.raftAppliedIndex (field 7, tag 0x38).
 * omitted / empty / dummy 0x00 = 0. leftover truncated
 * raftAppliedIndex is INVAL so a truncated status cannot look like
 * raftAppliedIndex 0. leftover length-delimited fields are skipped
 * by payload so leftover bytes cannot steal a printed
 * raftAppliedIndex. unknown wire types are INVAL. last
 * raftAppliedIndex is copied when raft_applied is set. */
...
int cetcd_parse_lease_ttl_granted(const uint8_t *req, size_t len,
                                  int64_t *granted);
/* leftover-safe LeaseTimeToLiveResponse.TTL (field 3, tag 0x18).
 * omitted / empty / dummy 0x00 = 0. leftover truncated remaining TTL
 * is INVAL so a truncated TTL cannot look like remaining 0. leftover
 * length-delimited fields are skipped by payload so leftover bytes
 * cannot steal a printed remaining TTL (tag 0x0a header revision is
 * not remaining TTL). unknown wire types are INVAL. last remaining
 * TTL is copied when ttl is set. */
...
int cetcd_parse_watch_event_kv_version(const uint8_t *req, size_t len,
                                       int64_t *version);
/* leftover-safe WatchResponse Event KV create_revision (field 2, tag
 * 0x10 inside Event field 2 / Watch field 11). omitted / empty /
 * dummy 0x00 = 0. leftover truncated Event-nested create_revision is
 * INVAL so a truncated watch cannot look like create_revision 0.
 * leftover length-delimited fields are skipped by payload so leftover
 * bytes cannot steal a printed create_revision (tag 0x0a header
 * member_id is not Event create_revision). unknown wire types are
 * INVAL. last Event create_revision is copied when create_rev is
 * set. */
...
int cetcd_parse_watch_event_kv_create_rev(const uint8_t *req, size_t len,
                                          int64_t *create_rev);
/* leftover-safe WatchResponse Event KV mod_revision (field 3, tag 0x18
 * inside Event field 2 / Watch field 11). omitted / empty / dummy
 * 0x00 = 0. leftover truncated Event-nested mod_revision is INVAL so
 * a truncated watch cannot look like mod_revision 0. leftover
 * length-delimited fields are skipped by payload so leftover bytes
 * cannot steal a printed mod_revision (tag 0x0a header revision is
 * not Event mod_revision). unknown wire types are INVAL. last Event
...
int cetcd_parse_watch_event_kv_mod_rev(const uint8_t *req, size_t len,
                                       int64_t *mod_rev);
/* leftover-safe WatchResponse Event KV value (field 5, tag 0x2a inside
 * Event field 2 / Watch field 11). omitted / empty / dummy 0x00 =
 * empty value. leftover truncated Event-nested value is INVAL so a
 * truncated watch cannot print leftover text as the Event value.
 * leftover length-delimited fields are skipped by payload so leftover
 * bytes cannot steal a printed Event value (tag 0x0a header is not
 * Event value). unknown wire types are INVAL. last Event value is
 * copied when value/value_cap are set. */
...
int cetcd_parse_watch_event_kv_value(const uint8_t *req, size_t len,
                                     char *value, size_t value_cap);
/* leftover-safe WatchResponse Event KV key (field 1, tag 0x0a inside
 * Event field 2 / Watch field 11). omitted / empty / dummy 0x00 =
 * empty key. leftover truncated Event-nested key is INVAL so a
 * truncated watch cannot print leftover text as the Event key.
 * leftover length-delimited fields are skipped by payload so leftover
 * bytes cannot steal a printed Event key (tag 0x0a header is not
 * Event key). unknown wire types are INVAL. last Event key is
...
int cetcd_parse_watch_event_prev_kv_create_rev(const uint8_t *req, size_t len,
                                               int64_t *create_rev);
/* leftover-safe WatchResponse Event prev_kv mod_revision (field 3,
 * tag 0x18 inside Event field 3 / Watch field 11). omitted / empty /
 * dummy 0x00 = 0. leftover truncated Event-nested prev_kv
 * mod_revision is INVAL so a truncated watch cannot look like prev
 * mod_revision 0. leftover length-delimited fields are skipped by
 * payload so leftover bytes cannot steal a printed prev
 * mod_revision (tag 0x0a header revision is not prev
...
int cetcd_parse_watch_event_prev_kv_mod_rev(const uint8_t *req, size_t len,
                                            int64_t *mod_rev);
/* leftover-safe WatchResponse Event prev_kv version (field 4, tag 0x20
 * inside Event field 3 / Watch field 11). omitted / empty / dummy
 * 0x00 = 0. leftover truncated Event-nested prev_kv version is INVAL
 * so a truncated watch cannot look like prev version 0. leftover
 * length-delimited fields are skipped by payload so leftover bytes
 * cannot steal a printed prev version (tag 0x0a header raft_term is
 * not prev version). unknown wire types are INVAL. last prev version
 * is copied when version is set. */
...
int cetcd_parse_watch_event_prev_kv_key(const uint8_t *req, size_t len,
                                        char *key, size_t key_cap);
/* leftover-safe WatchResponse Event prev_kv value (field 5, tag 0x2a
 * inside Event field 3 / Watch field 11). omitted / empty / dummy
 * 0x00 = empty value. leftover truncated Event-nested prev_kv value
 * is INVAL so a truncated watch cannot print leftover text as the
 * prev value. leftover length-delimited fields are skipped by payload
 * so leftover bytes cannot steal a printed prev value (tag 0x0a
 * header is not prev value). unknown wire types are INVAL. last prev
 * value is copied when value/value_cap are set. */
...
int cetcd_parse_delete_range_response(const uint8_t *req, size_t len,
                                      char *key, size_t key_cap, size_t *n);
/* leftover-safe DeleteRangeResponse.deleted (field 2, tag 0x10).
 * omitted / empty / dummy 0x00 = 0. leftover truncated deleted is
 * INVAL so a truncated del cannot print 0. leftover length-delimited
 * fields are skipped by payload so leftover bytes cannot steal a
 * printed delete count. unknown wire types are INVAL. */
...
int cetcd_parse_delete_range_prev_kv_value(const uint8_t *req, size_t len,
                                           char *value, size_t value_cap);
/* leftover-safe DeleteRangeResponse prev_kv lease (field 6, tag 0x30
 * inside field 3, tag 0x1a). omitted / empty / dummy 0x00 = 0.
 * leftover truncated prev_kv lease is INVAL so a truncated del cannot
 * look like lease 0. leftover length-delimited fields are skipped by
 * payload so leftover bytes cannot steal a printed prev lease (tag
 * 0x0a header is not prev lease). unknown wire types are INVAL. last
 * prev lease is copied when lease is set. */
