...
/* 0 → default 10s. Otherwise ceil(ms / tick_ms), at least 1 tick. */
CETCD_API int cetcd_v3rpc_watch_progress_ticks_from_ms(uint64_t interval_ms);
CETCD_API int cetcd_v3rpc_watch_progress_ticks_from_ms_tick(uint64_t interval_ms,
                                                            uint64_t tick_ms);
CETCD_API void cetcd_v3rpc_set_watch_progress_interval(uint64_t interval_ms,
                                                       uint64_t tick_ms);
...
CETCD_API int cetcd_apply_encode_compact(uint8_t **out, size_t *out_len,
                                         int64_t revision);
CETCD_API int cetcd_apply_encode_lease_grant(uint8_t **out, size_t *out_len,
                                             uint64_t lease_id, int64_t ttl);
CETCD_API int cetcd_apply_encode_lease_keepalive(uint8_t **out, size_t *out_len,
                                                 uint64_t lease_id, int64_t ttl);
...
CETCD_API int cetcd_apply_encode_auth_enabled(uint8_t **out, size_t *out_len,
                                              int enabled);
CETCD_API int cetcd_apply_encode_auth_user_delete(uint8_t **out, size_t *out_len,
                                                  const uint8_t *name,
                                                  size_t name_len);
CETCD_API int cetcd_apply_encode_auth_role_add(uint8_t **out, size_t *out_len,
                                               const uint8_t *name,
                                               size_t name_len);
...
CETCD_API int cetcd_apply_encode_auth_user_grant_role(uint8_t **out, size_t *out_len,
                                                      ...
                                                      const uint8_t *role,
                                                      size_t role_len);
CETCD_API int cetcd_apply_encode_auth_user_revoke_role(uint8_t **out, size_t *out_len,
                                                       const uint8_t *user,
                                                       size_t user_len,
                                                       const uint8_t *role,
                                                       size_t role_len);
CETCD_API int cetcd_apply_encode_auth_role_grant_perm(uint8_t **out, size_t *out_len,
                                                      const uint8_t *role,
                                                      size_t role_len,
                                                      const uint8_t *key,
                                                      size_t key_len,
...
#define CETCD_DEFAULT_WARNING_UNARY_MS 300
